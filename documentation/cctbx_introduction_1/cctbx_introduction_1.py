...
def plot_image(size1, size2, image_values, plot_file_name):
    ...
    image = numpy.reshape(numpy.array(image_values, dtype=float), (size2, size1))
...
def compute_reciprocal_space_distance_map(cbf_image):
  ...
  to the nearest reciprocal space node, measured in h, k, l space."""
    ...
    pixel = (detector.get_inferred_pixel_size(1), detector.get_inferred_pixel_size(2))
    ...
    for i in range(size[1]):
        for j in range(size[0]):
            ...
            d2 = sum([(_hkl[k] - hkl[k]) * (_hkl[k] - hkl[k]) for k in range(3)])
    ...
    try:
        plot_image(size[0], size[1], square_distances, "cctbx_introduction_1.png")
