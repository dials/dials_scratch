...
def model_reflection_rt0(reflection, experiment, params):
    ...
    if params.whole_panel:
        whole_panel = flex.double(
            flex.grid(p.get_image_size()[1], p.get_image_size()[0])
        )
    ...
    for i in range(nrays):
        if params.real_space_beam_simulation.enable:
            ...
            if (
                params.real_space_beam_simulation.illuminated_crystal_volume.shape
                == "sphere"
            ):
                crystal_radius = (
                    params.real_space_beam_simulation.illuminated_crystal_volume.sphere.radius
                )
                ...
                width = (
                    params.real_space_beam_simulation.illuminated_crystal_volume.rectangular_parallelepiped.width
                )
                depth = (
                    params.real_space_beam_simulation.illuminated_crystal_volume.rectangular_parallelepiped.depth
                )
        ...
        if still:
            result = predict_still_delpsi_and_s1(
                p0, experiment, b, s1_rotated=not params.interference_weighting.enable
            )
            if result is None:
    ...
    pred = p.get_ray_intersection(
        (matrix.col(experiment.beam.get_s0()) + (Amat * hkl)).normalize() * s0.length()
    )
    ...
    if params.debug:
        ...
        print(
            ...
            (
                matrix.col(reflection["xyzobs.mm.value"])
                - matrix.col(reflection["xyzsim.mm"])
            ).length()
            * 1000,
        )
        print(
    ...
    if params.plots:
        ...
        if params.whole_panel:
            ...
            ax.add_patch(
                patches.Rectangle(
                    (bbox[0], bbox[2]), bbox[1] - bbox[0], bbox[3] - bbox[2], fill=False
                )
            )
            plt.scatter(
            ...
            plt.scatter(
                [reflection["xyzcal.px"][0]], [reflection["xyzcal.px"][1]], c="red"
            )
            plt.scatter(
                [p.get_beam_centre_px(s0)[0]], [p.get_beam_centre_px(s0)[1]], c="gold"
            )
        ...
        plt.scatter(
            [reflection["xyzpred.mm"][0]], [reflection["xyzcal.mm"][1]], c="white"
        )
        plt.scatter([p.get_beam_centre(s0)[0]], [p.get_beam_centre(s0)[1]], c="gold")

        plt.scatter(
            [reflection["xyzsim.mm"][0]], [reflection["xyzsim.mm"][1]], c="purple"
        )

        plt.legend(["Obs", "Cal", "Pred", "BC", "Sim"])

        plt.plot(
            [reflection["xyzsim.mm"][0], p.get_beam_centre(s0)[0]],
            [reflection["xyzsim.mm"][1], p.get_beam_centre(s0)[1]],
            "k-",
            lw=2,
        )
...
def main(reflections, experiment, params):
    ...
    if params.debug:
        print(
            "RMSD obs - cal:",
            math.sqrt(
                (simmed["xyzobs.mm.value"] - simmed["xyzcal.mm"]).sum_sq() / len(simmed)
            )
            * 1000,
        )
        print(
            "RMSD obs - sim:",
            math.sqrt(
                (simmed["xyzobs.mm.value"] - simmed["xyzsim.mm"]).sum_sq() / len(simmed)
            )
            * 1000,
        )
