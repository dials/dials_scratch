...
class StillsExperimentalReflectionPredictor
    : public StillsDeltaPsiReflectionPredictor {
public:
  StillsExperimentalReflectionPredictor(
      const std::shared_ptr<BeamBase> beam, const Detector &detector,
      mat3<double> ub, const cctbx::uctbx::unit_cell &unit_cell,
      const cctbx::sgtbx::space_group_type &space_group_type,
      const double &dmin, double const &mos)
      : StillsDeltaPsiReflectionPredictor(beam, detector, ub, unit_cell,
                                          space_group_type, dmin),
        half_mosaicity_rad_(mos), ci_predict_ray_(beam->get_s0(), mos) {}

  virtual void append_for_index(stills_prediction_data &p,
                                const mat3<double> ub, const miller_index &h,
                                int panel = -1) {
    Ray ray;
