...
raise RuntimeError(
    "This module is out of date. Do not use it. It captures "
...
class ScanVaryingReflectionPredictor(object):
    ...
    def __init__(self, crystal, beam, gonio, scan, dmin):
        ...
        if crystal.num_scan_points == 0:
            raise TypeError(
                "The provided crystal has no scan point samples of the setting matrix"
            )
    ...
    def prepare(self, image_number, step=1):
        ...
        phi_beg = self._scan.get_angle_from_array_index(image_number, deg=False)
        phi_end = self._scan.get_angle_from_array_index(image_number + step, deg=False)
        r_beg = matrix.sqr(
        ...
        self._A1 = r_beg * self._crystal.get_A_at_scan_point(
            image_number - self._first_image
        )

        self._A2 = r_end * self._crystal.get_A_at_scan_point(
            image_number - self._first_image + step
        )
    ...
    def predict(self, hkl):
        ...
        starts_outside_ES = r1_from_ES >= 0.0
        ends_outside_ES = r2_from_ES >= 0.0
        ...
        if starts_outside_ES == ends_outside_ES:
            return None

        # stop prediction if the relp is outside the resolution limit
        if self.is_outside_res_limit:
            return None
...
class ScanVaryingReflectionListGenerator(object):
    ...
    def _search_on_image_range(self, indices):
        ...
        reflections = [
            e
            for t in range(indices[0], indices[1] + 1)
...
class AnglePredictor_py(object):
    ...
    def _predict_core(self, hkl):
        ...
        if r0mag_sq <= 1.0e-6:
            return None  # Relp is the recip. latt. origin
        if r0mag_sq > self._dstarmax_sq:
            return None  # Outside resolution limit
        ...
        if r0_beam_max > Cn_rad + self._s0mag:
            return None
        ...
        if r0_beam_max < self._s0mag - Cn_rad:
            return None
