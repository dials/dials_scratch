...
def vis(dp, header=True):
    ...
    print(
        "sensor_vertex1 <- sensor_origin + sensor_lim1[1] * sensor_d1 + sensor_lim2[1] * sensor_d2"
    )
    print(
        "sensor_vertex2 <- sensor_origin + sensor_lim1[2] * sensor_d1 + sensor_lim2[1] * sensor_d2"
    )
    print(
        "sensor_vertex3 <- sensor_origin + sensor_lim1[2] * sensor_d1 + sensor_lim2[2] * sensor_d2"
    )
    print(
        "sensor_vertex4 <- sensor_origin + sensor_lim1[1] * sensor_d1 + sensor_lim2[2] * sensor_d2"
    )
    print(
        "sensor_vertices <- matrix(data = c(sensor_vertex1,sensor_vertex2,sensor_vertex3,sensor_vertex4),ncol=3,byrow=T)"
    )
...
if __name__ == "__main__":
    ...
    p_vals = [110.0, 10.0, 10.0, pi / 12, pi / 12, pi / 12]
