...
import wx
from dials.viewer.viewer_utilities import (
    GetBitmap_from_np_array,
    from_wx_image_to_wx_bitmap,
)
import numpy
...
class ReflectionFrame(wx.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self.Image_01 = wx.StaticBitmap(
            self, bitmap=wx.EmptyBitmap(self.MaxImageSizeX, self.MaxImageSizeY)
        )
        self.Image_02 = wx.StaticBitmap(
            self, bitmap=wx.EmptyBitmap(self.MaxImageSizeX, self.MaxImageSizeY)
        )
        self.Image_03 = wx.StaticBitmap(
            self, bitmap=wx.EmptyBitmap(self.MaxImageSizeX, self.MaxImageSizeY)
        )
        ...
        h_box.Add(
            self.Image_01, 0, wx.ALIGN_CENTER_HORIZONTAL | wx.ALL | wx.ADJUST_MINSIZE, 7
        )

        h_box.Add(
            self.Image_02, 0, wx.ALIGN_CENTER_HORIZONTAL | wx.ALL | wx.ADJUST_MINSIZE, 7
        )

        h_box.Add(
            self.Image_03, 0, wx.ALIGN_CENTER_HORIZONTAL | wx.ALL | wx.ADJUST_MINSIZE, 7
        )
    ...
    def My_Update(self, request_new_size=True):
        if request_new_size == True:
            ...
            self.wx_Img_dat, self.img_width, self.img_height = GetBitmap_from_np_array(
                np_img_2d=self.dat, Intst_max=self.I_max, img_scale=self.frame_scale
            )

            self.wx_Img_bkg, self.img_width, self.img_height = GetBitmap_from_np_array(
                np_img_2d=self.bkg, Intst_max=self.I_max, img_scale=self.frame_scale
            )

            self.wx_Img_msk, self.img_width, self.img_height = GetBitmap_from_np_array(
                np_img_2d=self.msk, Intst_max=self.I_max, img_scale=self.frame_scale
            )

            self.Fit()
        else:
            self.My_Img_01 = from_wx_image_to_wx_bitmap(
                self.wx_Img_dat, self.img_width, self.img_height, self.frame_scale
            )

            self.Image_01.SetBitmap(self.My_Img_01)

            self.My_Img_02 = from_wx_image_to_wx_bitmap(
                self.wx_Img_bkg, self.img_width, self.img_height, self.frame_scale
            )

            self.Image_02.SetBitmap(self.My_Img_02)

            self.My_Img_03 = from_wx_image_to_wx_bitmap(
                self.wx_Img_msk, self.img_width, self.img_height, self.frame_scale
            )
