...
class Script(object):
    ...
    def list_possible_reflections(self, spacegroup, unit_cell, dmin, dmax):
        ...
        possible_indices_p1 = set(
            IndexGenerator(unit_cell, spacegroup.type(), dmin).to_array()
        )
        lowres_indices_p1 = set(
            IndexGenerator(unit_cell, spacegroup.type(), dmax + 1e-8).to_array()
        )
        ...
        systematic_present_indices = [
            n for n in possible_indices_p1 if not spacegroup.is_sys_absent(n)
        ]
        print(
            "%5d of these reflections are not systematically absent in %s"
    ...
    def StrategyEvaluator(self, experiments, evaluation_function_factory, dmin, dmax):
        ...
        def calculate_miller_rings(detector):
            ...
            all_scorings = {
                "%s:%s:%s"
                % (scan, str(x1), str(x2)): tools.geometric_scoring(
                    [hkl_to_id[hkl] for hkl in mrings[scan][x1][x2]], sc
                )["total"]
                for scan in mrings.iterkeys()
                for x1 in mrings[scan].iterkeys()
            ...
            print(
                ...
                sum(all_scorings.itervalues()) / len(list(all_scorings.itervalues())),
            )
            print(time.clock() - t)
        ...
        def calculate_observations(detector, goniometer, oscillation):
            ...
            t = time.clock()
            r = tools.determine_miller_ring_sectors(
                detector, goniometer, s0, possible_hkl_flex, crystal_A
            )
            ...
            print(
                map(
                    lambda x: tools.geometric_scoring(
                        [hkl_to_id[hkl] for hkl in x], sc
                    )["total"],
                    r,
                )
            )
            sc = tools.geometric_scoring([hkl_to_id[hkl] for hkl in r[4]], scorings)[
                "scorings"
            ]
            sc = tools.geometric_scoring([hkl_to_id[hkl] for hkl in r[5]], sc)[
                "scorings"
            ]
            sc = tools.geometric_scoring([hkl_to_id[hkl] for hkl in r[6]], sc)[
                "scorings"
            ]
            print(
                map(
        ...
        def export_mtz(observed_hkls, experiment, filename):
            ...
            batchnums = (
                _ for (x, n) in observed_hkls.iteritems() for _ in range(1, n + 1)
            )
            ...
            m.replace_original_index_miller_indices(
                cb_op_to_ref.apply(
                    cflex.miller_index(
                        [
                            _
                            for (x, n) in observed_hkls.iteritems()
        ...
        map_hkl_to_symmhkl = {
            r: rs for (r, rs) in zip(possible_hkl, list(asu_hkl_flex))
        }
        map_symmhkl_to_hkl = {}
        ...
        scoring_unique_reflection = {
            hkl: unique_id for (unique_id, hkl) in enumerate(possible_hkl)
        }
        scoring_unique_reflection = {
            hkl_to_id[h]: id for (h, id) in scoring_unique_reflection.iteritems()
        }
    ...
    def SimpleEvaluationFunctionFactory(
        self,
        possible_hkl,
        seen_hkl_multiplicity,
        map_hkl_to_symmhkl,
        map_symmhkl_to_hkl,
    ):
        # Asymptotic scoring.
        ...
        reflection_count = {
            hkl: seen_hkl_multiplicity[hkl] if hkl in seen_hkl_multiplicity else 0
            for hkl in possible_hkl
        }
        symm_reflection_count = {}
        for symm_hkl, symm_hkl_group in map_symmhkl_to_hkl.iteritems():
            symm_reflection_count[symm_hkl] = sum(
                [reflection_count[hkl] for hkl in symm_hkl_group]
            )
    ...
    def SimpleGeometricEvaluationFunctionFactory(
        self,
        possible_hkl,
        seen_hkl_multiplicity,
        map_hkl_to_symmhkl,
    ...
    def run(self):
        ...
        print(
            "Considering reflections with resolution %.2f - %.2f Ang"
            % (params.prediction.dmin, params.prediction.dmax)
        )

        evaluator = self.StrategyEvaluator(
            experiments,
...
if __name__ == "__main__":
    ...
    for p in d:
        ...
        rinv = two_theta_axis.axis_and_angle_as_r3_rotation_matrix(
            -10, deg=True
        ).transpose()
        rinv = matrix.sqr(
            rinv[0:3] + (0,) + rinv[3:6] + (0,) + rinv[6:9] + (0,) + (0, 0, 0, 1)
        ).transpose()
        ...
        r = two_theta_axis.axis_and_angle_as_r3_rotation_matrix(
            30, deg=True
        ).transpose()
        r = matrix.sqr(
            r[0:3] + (0,) + r[3:6] + (0,) + r[6:9] + (0,) + (0, 0, 0, 1)
        ).transpose()
