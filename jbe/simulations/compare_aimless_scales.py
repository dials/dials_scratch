...
def R(calc, obs):
    ...
    return sum(
        [math.fabs(math.fabs(o) - math.fabs(scale * c)) for c, o in zip(calc, obs)]
...
def read_aimless(unmerged_mtz):
    ...
    for hkl, x, y, z, Isigma, LP, scale in zip(
        indices,
        xdet.data(),
        ydet.data(),
        rot.data(),
        ...
        observations.append(
            (hkl, (x, y, z), [Isigma[1:2][0], Isigma[2:][0]], 1 / scale)
        )
    return observations
...
def main(args):
    ...
    plt.title(
        """Comparison inverse scale factors from aimless and
