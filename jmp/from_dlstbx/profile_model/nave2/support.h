...
public:
   ...
   */
  Support(const Beam &beam, const Detector &detector,
          const Goniometer &goniometer, const Scan &scan, const mat3<double> &A,
          const vec3<double> &sig_s, const vec3<double> &sig_a,
          const vec3<double> &sig_w, double p)
      : detector_(detector), scan_(scan), A_(A), s0_(beam.get_s0()),
        m2_(goniometer.get_rotation_axis()), sig_s_(sig_s), sig_a_(sig_a),
        sig_w_(sig_w), chi2p_(quantile(boost::math::chi_squared(3), p)) {
   ...
   */
  void compute_mask(std::size_t panel, vec3<double> s1, double phi0,
                    Shoebox<> &sbox) const {
   ...
   */
  void compute_prof(std::size_t panel, vec3<double> s1, double phi0, int6 bbox,
   ...
   */
  void compute_image_mask(af::ref<int, af::c_grid<2>> mask, std::size_t panel,
                          int z, vec3<double> s1, double phi0,
                          int6 bbox) const {
    ...
    if (y0 < 0)
      y0 = 0;
    if (x1 > mask.accessor()[1])
      x1 = mask.accessor()[1];
    if (y1 > mask.accessor()[0])
      y1 = mask.accessor()[0];
