...
def multihist(unmerged_mtz):
    ...
    hist_acentric = flex.histogram(
        mult_acentric.as_double(), data_min=0, data_max=max_mult, n_slots=max_mult
    )
    hist_centric = flex.histogram(
        mult_centric.as_double(), data_min=0, data_max=max_mult, n_slots=max_mult
    )
    for s, a, c in zip(
        hist_acentric.slot_centers(), hist_acentric.slots(), hist_centric.slots()
    ):
        print(s, a, c)
