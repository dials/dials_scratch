...
def mean_difference_over_sum(self, other, binner=None):
    ...
    if binner is not None:
        ...
        return (
            min(self.indices().size() / max(1, complete_set.indices().size()), 1.0)
            * multiplier
        )
    assert self.binner() is not None
    data = []
    for n_given, n_complete in zip(
        self.binner().counts_given(), self.binner().counts_complete()
    ):
        if n_complete == 0:
            data.append(return_fail)
        else:
            data.append(multiplier * n_given / n_complete)
