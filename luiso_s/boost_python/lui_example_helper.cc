...
void luiso_s_scratch_ext() {
  ...
  def("fitting_2d", &fitting_2d,
      (arg("descriptor"), arg("data2d"), arg("background2d"),
       arg("profile2d")));

  def("model_2d", &model_2d,
      (arg("nrow") = 100, arg("ncol") = 100, arg("a") = 10, arg("b") = 20,
       arg("delta_ang") = 1, arg("imax") = 50, arg("asp") = 0.5));

  def("measure_2d_angl", &measure_2d_angl,
      (arg("data2d"), arg("mask2d"), arg("xpos") = 1, arg("ypos") = 20));
  def("tst_prod", &tst_prod, arg("matr01"), arg("matr02"));
  def("tst_ref_prod", &tst_ref_prod, arg("matr01"), arg("matr02"));
  def("test_compare_2d", &test_compare_2d, arg("descriptor"), arg("data2d"),
      arg("total"));
}
