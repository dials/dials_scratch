...
class Script(object):
    ...
    def __init__(self):
      ...
      integrate
      {
        ...
        usage = (
            "usage: dials.python {0} [options] [param.phil] "
    ...
    def run(self):
        ...
        if have_data.count(False) > 0:
            no_data = [
                e for e, h in zip(self.params.data_directory, have_data) if not h
            ]
    ...
    def _run_one_job(self, command, keywords=None):
        ...
        try:
            result = easy_run.fully_buffered(
                command=command, stdin_lines=keywords
            ).raise_if_errors()
        except RuntimeError:
            print("Failed job")
            return None
    ...
    def _get_aimless_summary(logfile):
        ...
        with open(logfile, "r") as f:
            for line in f:
                if line.startswith("<!--SUMMARY_BEGIN-->"):
                    break
            for line in f:
    ...
    def process(self, job):
        ...
        if self._run_one_job(cmd) is not None:
            ...
            print(
                "{0} reflections indexed in P1 with unit cell ({1})".format(
                    n_indexed, cell_s
                )
            )
        ...
        if best_indexing is None:
            return
        print("Selecting {0} and {1} for further processing".format(*best_indexing))

        # Convert best indexing solution to P1 and P222 versions
        cmd = (
            "dials.reindex {0} {1} space_group={2} "
            "output.experiments=best_experiments_conv_to_{2}.json "
            "output.reflections=best_reflections_conv_to_{2}.pickle "
        )
        if (
            self._run_one_job(cmd.format(best_indexing[0], best_indexing[1], "P1"))
            is None
        ):
            return
        if (
            self._run_one_job(cmd.format(best_indexing[0], best_indexing[1], "P222"))
            is None
        ):
            return
        ...
        for sub_job in ("P1", "P222"):
            try:
                with cd(sub_job):
                    ...
                    print(
                        "Processing best indexing solution converted to {0}".format(
                            sub_job
                        )
                    )
    ...
    def _refine_onwards(self, experiments, reflections):
        ...
        cmd = (
            "dials.refine {0} {1} max_iterations=100 "
            "output.experiments=static_01_refined_experiments.json "
            "output.reflections=static_01_refined_reflections.pickle "
            "output.log=static_01_dials.refine.log"
        ).format(exp_path, ref_path)
        if self._run_one_job(cmd) is None:
            return
        cmd = (
            "dials.refine static_01_refined_experiments.json "
        ...
        print(
            "Detector origin:  ({0:.5f}, {1:.5f}, {2:.5f})".format(*d["origin"].elems)
        )
        ...
        cmd = (
            "dials.refine static_02_refined_experiments.json "
        ...
        # integrate
        cmd = (
            "dials.integrate sv_refined_experiments.json "
            "sv_refined_reflections.pickle nproc={0} "
            "prediction.d_min={1}"
        ).format(self.params.nproc, self.params.integrate.d_min)
        if self._run_one_job(cmd) is None:
            return
