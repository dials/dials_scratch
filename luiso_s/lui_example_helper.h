...
tst_ref_prod(af::const_ref<double, af::c_grid<2>> matr01,
...
af::versa<double, af::c_grid<2>>
tst_prod(af::const_ref<double, af::c_grid<2>> matr01,
    ...
    for (int row = 0; row <= nrow_in - 1;row++) {
      for (int col = 0; col <= ncol_in - 1;col++) {
        ...
        if (tot_row >= 0 and tot_col >= 0 and tot_row < nrow_tot and
          ...
          data2dreturn(tot_row, tot_col)=total(tot_row, tot_col) + data2d(row,
  col) * scale * x_contrib * y_contrib; if( xpos_ex != tot_col or ypos_ex !=
  tot_row ){ if( xpos_ex != tot_col ){ data2dreturn(tot_row,
  xpos_ex)=total(tot_row, xpos_ex)
          ...
          std::cout <<" centr_col =" <<  centr_col << " centr_row =" <<
  centr_row << "\n";
        }
      }
    }
  ...
  for (int row = 0; row < nrow_in; row++) {
    for (int col = 0; col < ncol_in; col++) {
      ...
      if (tot_row >= 0 and tot_col >= 0 and tot_row < nrow_tot and
        ...
        total(pos_tot_row, pos_tot_col) +=
            data2d(row, col) * scale * x_contrib * y_contrib;
        if (xpos_ex != tot_col or ypos_ex != tot_row) {
          if (xpos_ex != tot_col) {
            total(pos_tot_row, xpos_ex) +=
                data2d(row, col) * scale * (1 - x_contrib) * y_contrib;
          }
          if (ypos_ex != tot_row) {
            total(ypos_ex, pos_tot_col) +=
                data2d(row, col) * scale * x_contrib * (1 - y_contrib);
          }
          if (xpos_ex != tot_col and ypos_ex != tot_row) {
            total(ypos_ex, xpos_ex) +=
                data2d(row, col) * scale * (1 - x_contrib) * (1 - y_contrib);
          }
        }
...
vec2<double> fitting_2d(flex_double descriptor, flex_double data2d,
  ...
  for (int row = 0; row < nrow; row++) {
    for (int col = 0; col < ncol; col++) {
      if (data2dmov(row, col) != backg2dmov(row, col) and
          profile2d(row, col) > 0) {
...
flex_double model_2d(int nrow, int ncol, float a, float b, float delta_ang,
                     float imax, float asp) {
...
float measure_2d_angl(flex_int &data2d, flex_int &mask2d, float xpos,
                      float ypos) {
  int ncol = data2d.accessor().all()[1];
  int nrow = data2d.accessor().all()[0];
  std::cout << "\n"
  ...
  for (int pos = 0; pos < pie_size; pos++) {
    if (angl[pos] == -1 and dist[pos] == -1) {
      ...
      if (angl[next_pos] == -1 and dist[next_pos] == -1 and
          angl[prev_pos] == -1 and dist[prev_pos] == -1) {
        angl[pos] = float(pos) / pie_size;
        std::cout << "_________________________________________________________"
                     "__ exeption";
      } else if (angl[next_pos] != -1 and dist[next_pos] != -1 and
                 angl[prev_pos] != -1 and dist[prev_pos] != -1) {
  ...
  if (fin_pos == -1) {
    std::cout
        << "____________________________________________________ -1 ang\n";
  }
...
flex_double test_compare_2d(flex_double descriptor, flex_double data2d,
                            flex_double total) {
  flex_double data2dreturn(total);
  int ncol_in = data2d.accessor().all()[1];
  int nrow_in = data2d.accessor().all()[0];
