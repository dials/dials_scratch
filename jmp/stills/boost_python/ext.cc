...
template <typename T> T median(const af::const_ref<T> &x) {
...
public:
  ...
  Model(const Beam &beam, const Detector &detector, const Crystal &crystal,
        af::reflection_table reflections,
        const af::const_ref<int, af::c_grid<2>> &input_data,
        const af::const_ref<bool, af::c_grid<2>> &input_mask, double mosaicity,
        double bandpass, double foreground_limit, double background_limit,
        std::size_t num_samples, bool predict_all, bool use_mosaicity_for_mask)
      : beam_(beam), detector_(detector), crystal_(crystal),
        reflections_(reflections), image_data_(input_data.accessor()),
        image_mask_(input_mask.accessor()), mosaicity_(mosaicity),
        bandpass_(bandpass), foreground_limit_(foreground_limit),
        background_limit_(background_limit), num_samples_(num_samples),
        predict_all_(predict_all),
        use_mosaicity_for_mask_(use_mosaicity_for_mask),
        update_pixel_lookup_(true), update_image_model_(true),
        update_reflection_data_(true), first_(true), wavelength_values_(5),
  ...
  Beam get_beam() const { return beam_; }
  ...
  Detector get_detector() const { return detector_; }

  void set_detector(const Detector &detector) {
    DIALS_ASSERT(
        detector[0].get_image_size().all_eq(detector_[0].get_image_size()));
    detector_ = detector;
    update_pixel_lookup_ = true;
    update_image_model_ = true;
    update_reflection_data_ = true;
  ...
  Crystal get_crystal() const { return crystal_; }
  ...
  af::reflection_table get_reflections() const { return reflections_; }
  ...
  af::versa<int, af::c_grid<2>> get_image_data() const { return image_data_; }
  ...
  af::versa<int, af::c_grid<2>> get_image_mask() const { return image_mask_; }
  ...
  double get_mosaicity() const { return mosaicity_; }
  ...
  double get_bandpass() const { return bandpass_; }
  ...
  double get_foreground_limit() const { return foreground_limit_; }
  ...
  double get_background_limit() const { return background_limit_; }
  ...
  double get_num_samples() const { return num_samples_; }
  ...
  af::shared<vec3<double>> observed() const { return observed_; }

  af::shared<vec3<double>> predicted() const { return predicted_; }

  af::shared<double> background() const { return background_; }

  af::shared<double> intensity() const { return intensity_; }

  af::shared<double> variance() const { return variance_; }

  af::shared<std::size_t> num_foreground() const { return num_foreground_; }

  af::shared<std::size_t> num_background() const { return num_background_; }

  af::shared<double> scale() const { return scale_; }

  af::shared<bool> success() const { return success_; }
  ...
  std::size_t size() const { return intensity_.size(); }
  ...
  af::shared<double> wavelength_weights() const { return wavelength_weights_; }
...
protected:
  void generate_pixel_index_lookup() {
    ...
    for (std::size_t j = 0; j < ysize; ++j) {
      for (std::size_t i = 0; i < xsize; ++i) {
        ...
        vec3<double> h0(std::floor(h[0] + 0.5), std::floor(h[1] + 0.5),
                        std::floor(h[2] + 0.5));
    ...
    if (predict_all_) {
      ...
      for (LookupMap::iterator it = lookup_.begin(); it != lookup_.end();
           ++it) {
  ...
  void generate_image_model() {
    ...
    af::const_ref<cctbx::miller::index<>> miller_index =
        reflections_["miller_index"];
    ...
    for (std::size_t l = 0; l < miller_index.size(); ++l) {
      ...
      vec3<double> h0(miller_index[l][0], miller_index[l][1],
                      miller_index[l][2]);
      ...
      for (std::size_t k = 0; k < pixels.size(); ++k) {
        ...
        for (std::size_t jj = 0; jj < num_samples_; ++jj) {
          for (std::size_t ii = 0; ii < num_samples_; ++ii) {
            vec3<double> q =
                transform.q(0, i + (ii + 0.5) / (double)num_samples_,
        ...
        // to me!
        //
        // Thinking about it, this is probably wrong answer - I bet it has
        // something to do with the ratio between the areas which I'm missing
        // somewhere.
        /* I *= mosaicity_; */
        //
  ...
  void integrate_reflections() {
    ...
    for (std::size_t i = 0; i < reflections_.size(); ++i) {
      ...
      try {
        ...
        double B = compute_background(reflection_data.const_ref(),
                                      reflection_mask.const_ref());

        // Compute the intensity
        vec2<double> IV = compute_intensity(reflection_data.const_ref(),
                                            reflection_mask.const_ref(), B);

        double I = IV[0];
        double V = IV[1];
        double S = compute_scale(reflection_pred.const_ref(),
                                 reflection_mask.const_ref());

        vec3<double> xcal = compute_centroid(
            reflection_pred.const_ref(), reflection_mask.const_ref(),
            reflection_pnts_double.const_ref());

        vec3<double> xobs = compute_centroid(
            reflection_data.const_ref(), reflection_mask.const_ref(),
            reflection_pnts_double.const_ref());
  ...
  vec3<double> compute_centroid(af::const_ref<double> data,
  ...
  double compute_background(af::const_ref<double> data,
    ...
    // Compute the result
    RobustPoissonMean result(Y.const_ref(), median, tuning_constant, 1e-3,
                             max_iter);
  ...
  vec2<double> compute_intensity(af::const_ref<double> data,
                                 af::const_ref<int> mask, double B) const {
  ...
  double compute_scale(af::const_ref<double> pred,
                       af::const_ref<int> mask) const {
    double result = 0;
    int mask_code = Valid | Foreground;
...
BOOST_PYTHON_MODULE(dials_scratch_jmp_stills_ext) {
  class_<Model>("Model", no_init)
      .def(init<const Beam &, const Detector &, const Crystal &,
                af::reflection_table, const af::const_ref<int, af::c_grid<2>> &,
                const af::const_ref<bool, af::c_grid<2>> &, double, double,
                double, double, std::size_t, bool, bool>(
          (arg("beam"), arg("detector"), arg("crystal"), arg("reflections"),
           arg("image_data"), arg("image_mask"), arg("mosaicity") = 0.1,
           arg("bandpass") = 0.01, arg("foreground_limit") = 3.0,
           arg("background_limit") = 5.0, arg("num_samples") = 5,
           arg("predict_all") = false, arg("use_mosaicity_for_mask") = false)))
      .add_property("beam", &Model::get_beam, &Model::set_beam)
      .add_property("detector", &Model::get_detector, &Model::set_detector)
      .add_property("crystal", &Model::get_crystal, &Model::set_crystal)
      .add_property("reflections", &Model::get_reflections,
                    &Model::set_reflections)
      .add_property("image_data", &Model::get_image_data,
                    &Model::set_image_data)
      .add_property("image_mask", &Model::get_image_mask,
                    &Model::set_image_mask)
      .add_property("image_pred", &Model::get_image_pred)
      .add_property("mosaicity", &Model::get_mosaicity, &Model::set_mosaicity)
      .add_property("bandpass", &Model::get_bandpass, &Model::set_bandpass)
      .add_property("foreground_limit", &Model::get_foreground_limit,
                    &Model::set_foreground_limit)
      .add_property("background_limit", &Model::get_background_limit,
                    &Model::set_background_limit)
      .add_property("num_samples", &Model::get_num_samples,
                    &Model::set_num_samples)
      .def("update", &Model::update, (arg("pixel_lookup") = true))
      .def("least_squares_score", &Model::least_squares_score)
      .def("maximum_likelihood_score", &Model::maximum_likelihood_score)
  ...
  class_<StillsSimulator>("StillsSimulator", no_init)
      .def(init<const Beam &, const Detector &, const Crystal &, mat3<double>,
                mat3<double>, mat3<double>, std::size_t>())
      .def("normal_pdf", &StillsSimulator::normal_pdf)
      .def("pixel_area", &StillsSimulator::pixel_area)
      .def("integrate_pixel", &StillsSimulator::integrate_pixel);
}
