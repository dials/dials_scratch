...
def simulate_reflection(
  ...
  """
    ...
    if rlp_covariance is not None:
        r0 = matrix.col(
            multivariate_normal(r0, matrix.sqr(rlp_covariance).as_list_of_lists())
        )
    ...
    predictor = ScanStaticRayPredictor(s0, m2, fixed_rotation, setting_rotation, dphi)
...
if __name__ == "__main__":
    ...
    parser = OptionParser(
        usage=usage, phil=phil_scope, read_experiments=True, read_reflections=True
    )
