...
public:
  class Transform {
  public:
    Transform(int4 bbox, const CoordinateSystem &cs) : bbox_(bbox), cs_(cs) {}
  ...
  ProfileDrizzler(int3 grid_size, double fraction)
      : profile_(grid_size), profile_weight_(grid_size), fraction_(fraction) {}

  af::versa<double, af::c_grid<2>> profile() const { return profile_; }

  af::versa<double, af::c_grid<2>> weight() const { return profile_weight_; }

  void add(const af::const_ref<double, af::c_grid<2>> &image,
           const af::const_ref<double, af::c_grid<2>> &weight,
           const CoordinateSystem &cs) {
    drizzle(profile_.ref(), profile_weight_.ref(), image, weight, fraction_,
            make_transform(cs));
  }
