...
class MLPoisson2 {
public:
  MLPoisson2(const af::const_ref<double> &c, const af::const_ref<double> &s1,
             const af::const_ref<double> &s2, double S1, double S2, double step)
      : c_(c.begin(), c.end()), s1_(s1.begin(), s1.end()),
        s2_(s2.begin(), s2.end()) {
    DIALS_ASSERT(step > 0);
...
public:
  MLPoisson2Objective(const af::const_ref<double> &c,
...
public:
  MLPoisson2Stepper(const af::const_ref<double> &c,
                    const af::const_ref<double> &s1,
                    const af::const_ref<double> &s2, vec2<double> x)
      : obj_(c, s1, s2), X0_(x), B_(1, 0, 0, 1) {}
  ...
  void step() {
    ...
    vec2<double> ST = S;
    double YTS = YT * S;
    mat2<double> SYT(S[0] * YT[0], S[0] * YT[1], S[1] * YT[0], S[1] * YT[1]);
    mat2<double> YST(Y[0] * ST[0], Y[0] * ST[1], Y[1] * ST[0], Y[1] * ST[1]);
    mat2<double> SST(S[0] * ST[0], S[0] * ST[1], S[1] * ST[0], S[1] * ST[1]);
    B_ = (I - SYT / YTS) * B_ * (I - YST / YTS) + SST / YTS;
    /* std::cout << SYT[0] << ", " << SYT[1] << ", " << SYT[2] << ", " << SYT[3]
     * << std::endl; */
    /* std::cout << YST[0] << ", " << YST[1] << ", " << YST[2] << ", " << YST[3]
     * << std::endl; */
    /* std::cout << SST[0] << ", " << SST[1] << ", " << SST[2] << ", " << SST[3]
     * << std::endl; */
    /* std::cout << B_[0] << ", " << B_[1] << ", " << B_[2] << ", " << B_[3] <<
     * std::endl; */
    X0_ = X1;
  }

  vec2<double> X() const { return X0_; }

  mat2<double> B() const { return B_; }
...
private:
  ...
  double zoom(vec2<double> P, double alpha0, double alpha1, double f00,
              double d00, double f0) const {
    double c1 = 1e-4;
    double c2 = 0.9;
