...
class PredRelpViewer(ReciprocalLatticeViewer):
        ...
        for i_panel in range(len(detector)):
            ...
            if goniometer is not None:
                spots_mm["rlp"].set_selected(
                    sel,
                    S.rotate_around_origin(goniometer.get_rotation_axis(), -rot_angle),
                )
            else:
    ...
    def map_points_to_reciprocal_space(self):
        ...
        if self.reflections2 is not None:
            ...
            self.map_centroids_to_reciprocal_space(
                reflections2, self.detector, self.beam, goniometer
            )
            points2 = reflections2["rlp"] * 100
            self.viewer.set_points2(points2)

    def create_viewer_panel(self):
        self.viewer = MyGLWindow2(
...
def run(args):
    ...
    print(
        "{0} reflections from the second set do not match the first".format(
            len(reflections2)
        )
    )
