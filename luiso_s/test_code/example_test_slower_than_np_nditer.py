...
if __name__ == "__main__":
    ...
    for col in range(len(b_red[0, :])):
        for row in range(len(b_red[:, 0])):
            b_red[row, col] = red_func(
                b_red[row, col], ints_min, rim_n1, rim_n2, ints_max, block_size
            )

    for col in range(len(b_green[0, :])):
        for row in range(len(b_green[:, 0])):
            b_green[row, col] = green_func(
                b_green[row, col], ints_min, rim_n1, rim_n2, ints_max, block_size
            )

    for col in range(len(b_blue[0, :])):
        for row in range(len(b_blue[:, 0])):
            b_blue[row, col] = blue_func(
                b_blue[row, col], ints_min, rim_n1, rim_n2, ints_max, block_size
            )
