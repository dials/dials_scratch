...
class ViewerCanvas(MyCanvasBase):
    ...
    def OnDraw(self):
        ...
        for i in range(num_panels):
            ...
            for j in [0, 2, 3, 4, 5]:
                glNormal3f(d_r_f[i][j][0, 0], d_r_f[i][j][0, 1], d_r_f[i][j][0, 2])
                # For each vertex on the face
                for k in range(1, 5):
                    glVertex3f(d_r_f[i][j][k, 0], d_r_f[i][j][k, 1], d_r_f[i][j][k, 2])
        glEnd()
        ...
        glVertex3f(d_r_f[0][1][1, 0], d_r_f[0][1][1, 1], d_r_f[0][1][1, 2])
        glTexCoord2f(1.0, 0.0)
        glVertex3f(d_r_f[0][1][2, 0], d_r_f[0][1][2, 1], d_r_f[0][1][2, 2])
        glTexCoord2f(1.0, 1.0)
        glVertex3f(d_r_f[0][1][3, 0], d_r_f[0][1][3, 1], d_r_f[0][1][3, 2])
        glTexCoord2f(0.0, 1.0)
        glVertex3f(d_r_f[0][1][4, 0], d_r_f[0][1][4, 1], d_r_f[0][1][4, 2])
        glEnd()
...
class MainWindow(wx.Frame):
    def __init__(self, parent=None, id=-1, title="CBFviewer"):
        ...
        editmenu = wx.Menu()
        opsmenu = wx.Menu()
        menuitem = opsmenu.Append(
            -1, "Adjust &detector\tCtrl+D", "Bring up detector orientation dialog"
        )
        self.Bind(wx.EVT_MENU, self.OnAdjustDet, menuitem)
        menuitem = opsmenu.Append(
            -1, "Adjust &sample\tCtrl+S", "Bring up sample orientation dialog"
        )
        self.Bind(wx.EVT_MENU, self.OnAdjustSam, menuitem)
        menuitem = opsmenu.Append(
            -1, "Adjust &goniometer\tCtrl+G", "Bring up goniometer orientation dialog"
        )
        self.Bind(wx.EVT_MENU, self.OnAdjustGonio, menuitem)
        editmenu.AppendMenu(wx.ID_PREFERENCES, "&Options", opsmenu)
        menuitem = editmenu.Append(
            -1, "&Reset\tCtrl+R", "Reset parameters to original values"
        )
        self.Bind(wx.EVT_MENU, self.OnReset, menuitem)
        menuitem = editmenu.Append(
            -1, "&Shadow graph", "Bring up affected pixel plotting dialog"
        )
        ...
        menuitem = viewmenu.Append(
            -1, "x-y\tCtrl+1", "Change viewing projection to be parallel to x-y plane"
        )
        self.Bind(wx.EVT_MENU, self.xy, menuitem)
        menuitem = viewmenu.Append(
            -1, "x-z\tCtrl+2", "Change viewing projection to be parallel to x-z plane"
        )
        self.Bind(wx.EVT_MENU, self.xz, menuitem)
        menuitem = viewmenu.Append(
            -1, "y-z\tCtrl+3", "Change viewing projection to be parallel to y-z plane"
        )
        self.Bind(wx.EVT_MENU, self.yz, menuitem)
        menuitem = viewmenu.Append(
            -1, "y-x\tCtrl+4", "Change viewing projection to be parallel to y-x plane"
        )
        self.Bind(wx.EVT_MENU, self.yx, menuitem)
        menuitem = viewmenu.Append(
            -1, "z-x\tCtrl+5", "Change viewing projection to be parallel to z-x plane"
        )
        self.Bind(wx.EVT_MENU, self.zx, menuitem)
        menuitem = viewmenu.Append(
            -1, "z-y\tCtrl+6", "Change viewing projection to be parallel to z-y plane"
        )
    ...
    def OnAbout(self, event):
        message = (
            "CBFViewer uses PyOpenGL in wxPython to display a"
...
class DetectorDialog(wx.Dialog):
    ...
    def InitUI(self):
        ...
        self.detcheck = wx.CheckBox(panel, -1, "Enable detector texture", (40, 200))
    ...
    def OnAdjust(self, event):
        ...
        for i in range(num_panels):
            ...
            detector_origin[i] = detector_origin[i] + displacement * detector_normal
            detector_origin[i] = np.dot(r_x, detector_origin[i])
            fast_axis[i] = np.dot(r_x, fast_axis[i])
            slow_axis[i] = np.dot(r_x, slow_axis[i])
            d_r_f[i] = det[i].get_coords(detector_origin[i], fast_axis[i], slow_axis[i])
        ...
        [
            ...
            tex_noshad,
            noshad_data,
        ] = newshadow.shadow_texture(kappa=kappa, omega=omega, blank=blanksetting)

        pixels = newshadow.affected_pixels(
            kappa=kappa, omega=omega, blank=blanksetting
        )[1]

        if shadowsetting is True:
            print("At these goniometer angles, the shadow affects", pixels, "pixels.")
...
class SampleDialog(wx.Dialog):
    ...
    def InitUI(self):
        ...
        wx.StaticText(
            panel,
            label="Enter new sample angles between " "-90.0 and 90.0 degrees. ",
            pos=(20, 20),
        )

        labels = [
            "Rotate sample about x axis (omega): ",
            "Rotate sample about y axis: ",
            "Rotate sample about z axis: ",
            "Sample position: ",
        ]
        ...
        self.rb1 = wx.RadioButton(panel, -1, "(0, 0, 0)", (220, 210), style=wx.RB_GROUP)
    ...
    def OnCompute(self, event):
        ...
        for i in range(num_panels):
            ...
            d_r_f[i] = newDet[i].get_coords(new_origin, fast_axis[i], slow_axis[i])
...
class GonioDialog(wx.Dialog):
    ...
    def InitUI(self):
        ...
        self.widget = dict()
        panel = wx.Panel(self)
        wx.StaticText(panel, label="Enter new goniometer parameters.", pos=(20, 20))
        ...
        self.shadowcheck = wx.CheckBox(panel, -1, "Enable goniometer shadow", (40, 350))
...
class PlotDialog(wx.Dialog):
    ...
    def InitUI(self):
        ...
        wx.StaticText(
            panel,
            label="Enter the number of points in "
            "each dimension. \n Note the "
        ...
        # angles widget
        self.points = wx.SpinCtrl(
            panel, -1, pos=(150, 75), min=10, max=10000, initial=50
        )
