...
for h1, h2, p1, p2 in zip(
    obs_indices_rstbx, obs_indices_wrap, obs_angles_rstbx, obs_angles_wrap
):
...
for hkl, phi in zip(obs_indices_rstbx, obs_angles_rstbx):
    ...
    if ap_phi is None:
        continue
    assert approx_equal(ap_phi[0], phi, out=None) or approx_equal(
        ap_phi[1], phi, out=None
    )

# Now test prediction of multiple indices from AnglePredictor_py
obs_indices_dgw, obs_angles_dgw = ap.observed_indices_and_angles_from_angle_range(
    phi_start_rad=0.0, phi_end_rad=math.pi, indices=indices
)
...
rotation_angles = RotationAngles(mybeam.get_s0(), mygonio.get_rotation_axis())
...
for h1, phi1, h2, phi2 in zip(
    obs_indices_wrap, obs_angles_wrap, dials_obs_indices, dials_angles
):
    assert h1 == h2
...
for h, ang, s_dir, ref in zip(hkls, angles, s_dirs, dials_reflections):
    ...
    assert approx_equal(s_dir, matrix.col(ref.beam_vector).normalize().elems)
...
impacts = [dials_panel.get_ray_intersection(e.beam_vector) for e in dials_reflections]
