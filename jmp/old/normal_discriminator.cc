...
template <typename FloatType> void normal_discriminator_suite() {

  def("maximum_n_sigma", &maximum_n_sigma<FloatType>, (arg("data")));

  def("minimum_n_sigma", &minimum_n_sigma<FloatType>, (arg("data")));

  def("absolute_maximum_n_sigma", &maximum_n_sigma<FloatType>, (arg("data")));

  def("is_normally_distributed", &is_normally_distributed_wrapper<FloatType>,
      (arg("data"), arg("n_sigma") = -1));
}

template <typename FloatType> struct NDOperator {
...
void export_normal_discriminator() {
  ...
  class_<NormalDiscriminator>("NormalDiscriminator", no_init)
      .def(init<std::size_t>((arg("min_data") = 10)))
      .def("__call__", call_with_mask_float, (arg("shoebox"), arg("mask")))
      .def("__call__", call_with_mask_double, (arg("shoebox"), arg("mask")))
      .def("__call__", call_without_mask_float, (arg("shoebox")))
      .def("__call__", call_without_mask_double, (arg("shoebox")));
}
