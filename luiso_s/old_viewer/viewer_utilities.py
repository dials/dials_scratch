...
class np_to_bmp(object):
    def __call__(
        self, np_img_2d, Intst_max, ofst, xyz, title="Aaaaaaaaaa", mask_colour=False
    ):
        lc_fig = plt.figure()
        ...
        if np_img_2d == None:
            ...
            self.np_buf = np.fromstring(lc_fig.canvas.tostring_rgb(), dtype=np.uint8)
