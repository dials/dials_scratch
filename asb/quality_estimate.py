...
class Script(object):
    ...
    def __init__(self):
        ...
        # The script usage
        usage = (
            "usage: %s [options] [param.phil] directory name"
        ...
        for exp_path, refl_path in zip(exp_paths, refl_paths):
            ...
            for exp_id, experiment in enumerate(experiments):
                ...
                for i in binner.range_used():
                    ...
                    avg_i = (
                        flex.mean(bin_refls["intensity.sum.value"])
                        if n_refls > 0
                        else 0
                    )
                    avg_i_sigi = (
                ...
                print(
                    ...
                    "unit cell:",
                    ", ".join(
                        ["%.2f" % p for p in crystal.get_unit_cell().parameters()]
                    ),
                    ...
                    if len(best_limits) < params.best_count:
                        ...
                        for t, data in best_data.iteritems():
                            if worst_d_min == data[0]:
                                ...
                                best_limits[
                                    flex.first_index(best_limits, worst_d_min)
                                ] = d_min
                                break
