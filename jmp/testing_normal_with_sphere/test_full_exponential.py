...
def compute_mean_estimate(peak_f, peak_fx, peak_fy, mu, sigma):
    ...
    H_g2 = -matrix.sqr(
        (
            ...
            0.5 * (sigma_inv[1] + sigma_inv[2]),
            0.5 * (sigma_inv[1] + sigma_inv[2]),
            sigma_inv[3] + 1 / peak_fy[1] ** 2,
        )
    )

    A = (
        (2 * pi)
        * (1 / sqrt((-H_f).determinant()))
        * exp(-0.5 * ((peak_f - mu).transpose() * sigma_inv * (peak_f - mu))[0])
    )

    B = (
        (2 * pi)
        * (1 / sqrt((-H_g1).determinant()))
        * peak_fx[0]
        * exp(-0.5 * ((peak_fx - mu).transpose() * sigma_inv * (peak_fx - mu))[0])
    )

    C = (
        (2 * pi)
        * (1 / sqrt((-H_g2).determinant()))
        * peak_fy[1]
        * exp(-0.5 * ((peak_fy - mu).transpose() * sigma_inv * (peak_fy - mu))[0])
    )
...
sigma = matrix.sqr((0.03, 0, 0, 0.01))
sigma_inv = sigma.inverse()
