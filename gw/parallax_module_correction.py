...
def image_to_XDS_XYCORR(image_filename, sensor_thickness_mm):
  ...
  correction table in 0.025 rather than 0.1 (original) pixel increments."""
    ...
    x_corrections_parallax = (
        read_xds_calibration_file("X-CORRECTIONS.cbf").as_double() / 40.0
    )
    y_corrections_parallax = (
        read_xds_calibration_file("Y-CORRECTIONS.cbf").as_double() / 40.0
    )
