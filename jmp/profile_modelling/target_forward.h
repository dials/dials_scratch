...
namespace dials { namespace algorithms {
  ...
  public:
    ...
    vec3<double> sample() {
      ...
      // COV = LL*
      mat3<double> L(
          parameters_[0], 0.0, 0.0,
          parameters_[1], parameters_[2], 0.0,
          parameters_[3], parameters_[4], parameters_[5]);

      // Generate a normal random variate
      return L * vec3<double>(
          normal_(generator_),
          normal_(generator_),
          normal_(generator_)) + vec3<double>(h0_[0], h0_[1], h0_[2]);
    }
  ...
  public:
    ...
    MLTargetForward3DSingle(
          ...
          BeamVectorDistribution beam_vector_distribution,
          std::size_t num_sample)
      : m2_(gonio.get_rotation_axis()),
        fixed_rotation_(gonio.get_fixed_rotation()),
        setting_rotation_(gonio.get_setting_rotation()),
        panel_(panel),
        scan_(scan),
        entering_(entering),
        bbox_(bbox),
        reciprocal_lattice_point_distribution_(reciprocal_lattice_point_distribution),
    ...
    double log_likelihood(
      ...
      scitbx::af::const_ref< double, scitbx::af::c_grid<3> > model = model_.const_ref();
    ...
    scitbx::af::versa< double, scitbx::af::c_grid<3> > simulate() {
      ...
      if (model_.size() == 0) {
        ...
        dials::algorithms::RayPredictor ray_predictor(m2_, fixed_rotation_, setting_rotation_);
