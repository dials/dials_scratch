...
class ScriptRunner(object):
    ...
    def __call__(self):
        ...
        if len(observations) != len(rlist):
            ...
            print(
                "Those reflections that do not intersect have been saved"
                " to {0}".format(unique_filename)
            )
            pickle.dump(
                observations, open(unique_filename, "wb"), pickle.HIGHEST_PROTOCOL
            )
        ...
        if self.output_filename is not None:
            ...
            pickle.dump(
                observations, open(self.output_filename, "wb"), pickle.HIGHEST_PROTOCOL
            )
