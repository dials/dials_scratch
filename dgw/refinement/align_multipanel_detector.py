...
if __name__ == "__main__":
    ...
    fast_offsets = [
        (matrix.col(p.get_origin()) - panel60_origin).dot(fast) for p in detector
    ]
    slow_offsets = [
        (matrix.col(p.get_origin()) - panel60_origin).dot(slow) for p in detector
    ]

    ref_origin = matrix.col(
        (-210.68270819092896, 205.69567063383158, -263.7163617113081)
    )
    ref_fast = matrix.col(
        (0.9999974877443938, -0.0015832096467848918, -0.0015868056325671835)
    )
    ref_slow = matrix.col(
        (-0.0015900007754938183, -0.9999895438652769, -0.004287663425368496)
    )
