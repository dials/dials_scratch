...
if len(sys.argv) != 4:
    sys.stderr.write("Usage: dials.python select_eiger_frames.py input_master.h5 output_master.h5 hits.lst\n\n")
    sys.stderr.write("hits.lst is the indices of hit images to be selected (1-indexed).\n")
    sys.stderr.write("One way to generate it is as follows:\n")
    sys.stderr.write("    dials.spot_counts_per_image imported.expt strong.refl json=strong.json > count.lst\n")
    sys.stderr.write("    awk 'NF==9 && 0+$4 >= 15 {print $2}' count.lst > hits.lst # 15 is the minimum number of spots\n\n") 
    sys.exit(-1)
...
for dataname in sorted(f["/entry/data"]):
    ...
    f_start = int(data.attrs["image_nr_low"]) - 1 # originally 1-indexed in the master file,
...
def copy_visitor(name, node):
    ...
    elif name == "entry/sample/goniometer/omega" or \
       name == "entry/sample/goniometer/omega_end" or \
       name == "entry/sample/transformations/omega" or \
       name == "entry/sample/transformations/omega_end":
        filtered = node[()][hits]
        new_data = g.create_dataset(name, data=filtered, compression="gzip", shuffle=True)
...
output_data = g.create_dataset("/entry/data/data_000001",
                               shape=(len(hits), ny, nx), maxshape=(None, ny, nx),
                               dtype=original_data.dtype, chunks=original_data.chunks,
                               compression=fid, compression_opts=fopt, allow_unknown_filter=True)
...
frame_index = g.create_dataset("/entry/original_frame_index", data=hits + 1, compression="gzip", shuffle=True) # 1 indexed

g["/entry/description"] = "This file was created by select_eiger_frames.py (Takanori Nakane, 2024). The frame indices in the original file are stored in /entry/original_frame_index (1-indexed)."
