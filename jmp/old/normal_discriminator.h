...
template <typename FloatType>
bool is_normally_distributed(const af::const_ref<FloatType> &data) {
  return is_normally_distributed<FloatType>(
      data, normal_expected_n_sigma(data.size()));
}
...
public:
   ...
   */
  NormalDiscriminator(std::size_t min_data) : min_data_(min_data) {
  ...
  void operator()(const af::const_ref<FloatType> &shoebox,
    ...
    for (; num_data > min_data_; --num_data) {
      if (is_normally_distributed(
              af::const_ref<FloatType>(pixels.begin(), num_data))) {
        break;
      }
    }
