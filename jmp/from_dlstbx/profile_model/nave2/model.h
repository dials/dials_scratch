...
public:
   ...
   */
  Model(mat3<double> D, mat3<double> A, vec3<double> s0, vec3<double> m2,
        vec3<double> s1, double phi, vec3<double> sig_s, vec3<double> sig_a,
        vec3<double> sig_w)
      : D_(D), A_(A), s0_(s0), m2_(m2.normalize()),
        s1_(s1.normalize() * s0.length()), phi0_(phi),
        rlp_(R(-phi) * (s1 - s0)) {
    ...
    mat3<double> sigma_s(sqr(sig_s[0]), 0, 0, 0, sqr(sig_s[1]), 0, 0, 0,
                         sqr(sig_s[2]));

    // The covariance matrix for the spread in unit cell size component in the
    // reciprocal lattice coordinate system
    vec3<double> h = A.inverse() * rlp_;
    mat3<double> sigma_a(sqr(h[0] * sig_a[0]), 0, 0, 0, sqr(h[1] * sig_a[1]), 0,
                         0, 0, sqr(h[1] * sig_a[2]));
    ...
    mat3<double> U(v2[0], v3[0], rn[0], v2[1], v3[1], rn[1], v2[2], v3[2],
                   rn[2]);

    // Compute the angular spread
    double w =
        ((A * mat3<double>(sig_w[0], 0, 0, 0, sig_w[1], 0, 0, 0, sig_w[2])) *
         rlp_)
            .length();

    // Construct an eigenvalue matrix
    mat3<double> V(w * w, 0, 0, 0, w * w, 0, 0, 0, 0);
  ...
  mat3<double> D() const { return D_; }

  /** @returns The A matrix */
  mat3<double> A() const { return A_; }
  ...
  vec3<double> rlp() const { return rlp_; }

  /** @returns The covariance matrix */
  mat3<double> sigma() const { return sigma_; }

  /** @returns The inverse covariance matrix */
  mat3<double> sigma_inv() const { return sigma_inv_; }
