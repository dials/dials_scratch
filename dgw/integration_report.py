...
def generate_integration_report(
  ...
  """
    ...
    def binned_report(binner, index, data):
        ...
        # Compute mean I summation
        try:
            report["i_sum"] = list(
                indexer_sum.mean(data["intensity.sum.value"].select(data["sum"]))
            )
        except Exception:
            report["i_sum"] = [0.0] * len(binner)
    ...
    resolution_binner = Binner(
        resolution_bins(
            experiment, data["miller_index"], n_resolution_bins, d_max, d_min
        )
    )
...
class Script(object):
    ...
    def run(self):
        ...
        report["d_mid"] = []
        for i in range(len(report["bins"]) - 1):
            report["d_mid"].append((report["bins"][i + 1] + report["bins"][i]) / 2.0)
            table.rows.append(
                [
                    "%.2f" % report["bins"][i + 1],
                    "%.2f" % report["d_mid"][i],
