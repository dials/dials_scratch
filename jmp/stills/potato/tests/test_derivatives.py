...
from dials_scratch.jmp.stills.potato.profile_model import ReflectionProfileModel
...
def test_dS_bar_db(reflection_model):
    ...
    dS_bar_db1_cal = reflection_model.conditional().first_derivatives()[0]
    dS_bar_db2_cal = reflection_model.conditional().first_derivatives()[1]
    dS_bar_db3_cal = reflection_model.conditional().first_derivatives()[2]
    dS_bar_db4_cal = reflection_model.conditional().first_derivatives()[3]
    dS_bar_db5_cal = reflection_model.conditional().first_derivatives()[4]
    dS_bar_db6_cal = reflection_model.conditional().first_derivatives()[5]
...
def test_d2S_bar_dbij(i, j, reflection_model):
    ...
    d2S_bar_db_cal = reflection_model.conditional().second_derivatives()[i][j]
...
def test_d2L_dbij(i, j, reflection_model):
    ...
    d2Ldb_cal = reflection_model.second_derivatives()
...
def test():
    ...
    for i in range(10):
        ...
        reflection_model = ReflectionProfileModel(parameterisation, s0, s2, ctot, Sobs)
