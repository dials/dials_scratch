...
from dials_scratch.jmp.potato.util.generate_simple import generate_with_wavelength_spread2
...
def log_likelihood(params, s0, s2_list, xbar_list, ctot_list, Sobs_list):
  ...
  for i in range(len(s2_list)):
    ...
    eigen_values_v = matrix.sqr((
      0, 0, 0,
      0, 0, 0,
      0, 0, sigma_vs))
    sigma_v = R0.transpose()*eigen_values_v*R0
    sigma_vq = s0*(v.transpose()*(sigma_q+q3*q3.transpose())-(v.dot(q3))*q3.transpose())
