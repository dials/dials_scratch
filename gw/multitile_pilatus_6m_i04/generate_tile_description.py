...
def print_offsets_in_cif(offsets):
    ...
    for row, column in sorted(offsets):
        ...
        print(
            "%s translation detector DETECTOR_X    1 0 0 %8.2f %8.2f   0.00"
            % (element_x, offset_x, offset_y)
        )
        print(
            "%s translation detector %s 0 1 0     0.00     0.00   0.00"
            % (element_y, element_x)
        )
        print(
            "%s rotation    detector %s 0 0 1     0.00     0.00   0.00"
            % (element_r, element_y)
        )
...
def print_offsets_in_cif2(offsets):
    ...
    for row, column in sorted(offsets):
        ...
        offset_y = row * (195 + 17)
        element = "MOD_R%02d_C%02d" % (row, column)
        array = "ARRAY1(%d:%d,%d:%d)" % (
            offset_x + 1,
            offset_x + 487,
            offset_y + 1,
            offset_y + 195,
        )
        print(" FRAME1 %s %-27s 1" % (element, array))
