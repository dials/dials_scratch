...
FloatType moment(const af::const_ref<FloatType> &data, FloatType c,
                 std::size_t k) {
...
public:
  ...
  void operator()(const af::const_ref<FloatType> &shoebox,
    ...
    for (; num_data > min_data_; --num_data) {
      if (is_poisson_distributed(
              af::const_ref<FloatType>(pixels.begin(), num_data), n_sigma_)) {
        break;
      }
    }
