...
template <typename FloatType> void poisson_discriminator_suite() {

  def("moment", &moment<FloatType>, (arg("data"), arg("centre"), arg("k")));

  def("is_poisson_distributed", &is_poisson_distributed<FloatType>,
      (arg("data"), arg("n_sigma")));
}

template <typename FloatType> struct PDOperator {
...
void export_poisson_discriminator() {
  ...
  class_<PoissonDiscriminator>("PoissonDiscriminator")
      .def(init<std::size_t, double>(
          (arg("min_data") = 10, arg("n_sigma") = 3.0)))
