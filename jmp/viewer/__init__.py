def show_image_stack(data, axis=0, interpolation="none", **kwargs):
    ...
    slider = Slider(
        ax, "Axis %i index" % axis, 0, data.shape[axis] - 1, valinit=0, valfmt="%i"
    )
...
def show_image_stack_multi_view(
    ...
    slider1 = Slider(
        ax4, "%s index" % axis_names[0], 0, data.shape[0] - 1, valinit=0, valfmt="%i"
    )
    slider2 = Slider(
        ax5, "%s index" % axis_names[1], 0, data.shape[1] - 1, valinit=0, valfmt="%i"
    )
    slider3 = Slider(
        ax6, "%s index" % axis_names[2], 0, data.shape[2] - 1, valinit=0, valfmt="%i"
    )
