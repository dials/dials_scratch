  ...
  fraction of the current parameter value."""
    ...
    def _set_param_vals(self, new_vals):
        ...
        uc_list = [
            ...
            for xl in self.original_experiments.crystals()
        ]
        ...
        if not self._perturb_cell_lengths:
            for uc, crystal in zip(uc_list, experiments.crystals()):
                ...
                params[2] = uc[2]
                newB = matrix.sqr(
                    unit_cell(params).fractionalization_matrix()
                ).transpose()
                crystal.set_B(newB)
    ...
    normal distribution with sigma equal to fraction*value"""
        ...
        new_vals = [
            random.gauss(mu, sig) for (mu, sig) in zip(self._old_vals, self._sigmas)
        ]
    ...
    def known_perturbation(self, shifts=[]):
        ...
        if len(shifts) != len(self._old_vals):
            raise RuntimeError(
                "Wrong number of parameter shifts. This experiment "
...
if __name__ == "__main__":
  ...
  """,
    ...
    parser = OptionParser(
        phil=phil_scope, usage=usage, read_experiments=True, check_format=False
    )
    ...
    log.config(
        params.refinement.verbosity,
        info="synthetic_refine.log",
        debug="synthetic_refine.debug.log",
    )

    info("Perturbing original experiments")
    exp_perturber = ExperimentsPerturber(
        experiments,
    ...
    if err.iid_xyzobs_sigmas != [0.0, 0.0, 0.0]:
        if xyzvar != [0.0, 0.0, 0.0]:
            raise RuntimeError(
    ...
    if err.units == "px":
        info(
            "Converting units for requested errors. Only the first experiment "
    ...
    print(
        "offset angle between fast axes is",
        old_fast.accute_angle(new_fast, deg=True),
        "degrees",
    )
    print(
        "offset angle between slow axes is",
        old_slow.accute_angle(new_slow, deg=True),
        "degrees",
    )
