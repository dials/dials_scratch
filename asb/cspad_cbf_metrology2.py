...
indexing_program = os.path.join(
    libtbx.env.find_in_repositories("dials_scratch"), "asb", "mp_index.py"
)
...
for i in xrange(8):
    ...
    if not os.path.exists(trial):
        os.makedirs(trial)
    command = "libtbx.python %s %s mp.nproc=64 %s output_dir=%s" % (
        indexing_program,
        data_path,
        indexing_phil,
        trial,
    ...
    if i > 0:
        ...
        geom_path = os.path.join(
            ...
            "metrology",
            prev_trial,
            "t%s_refined_experiments_level%d.json" % (prev_trial, prev_level),
        )
