...
public:
  StillsSimulator(const Beam &beam, const Detector &detector,
                  const Crystal &crystal, mat3<double> sigma_wavelength_spread,
                  mat3<double> sigma_angular_spread,
                  mat3<double> sigma_rlp_mosaicity, std::size_t num_points)
      : beam_(beam), detector_(detector), crystal_(crystal),
        ...
        sigma_rlp_mosaicity_(sigma_rlp_mosaicity), num_points_(num_points) {}
  ...
  double polygon_area(vec3<double> A, vec3<double> B, vec3<double> C,
                      vec3<double> D) const {
    return ((A - B).cross(A - D)).length() / 2.0 +
           ((C - B).cross(C - D)).length() / 2.0;
  }
  ...
  double pixel_area(std::size_t panel, double s0_length, int x, int y) const {
    ...
    vec3<double> s00 =
        ...
        s0_length;
    vec3<double> s01 = detector_[panel]
                           .get_pixel_lab_coord(vec2<double>(x + 1, y))
                           .normalize() *
                       s0_length;
    vec3<double> s10 = detector_[panel]
                           .get_pixel_lab_coord(vec2<double>(x, y + 1))
                           .normalize() *
                       s0_length;
    vec3<double> s11 = detector_[panel]
                           .get_pixel_lab_coord(vec2<double>(x + 1, y + 1))
                           .normalize() *
                       s0_length;
  ...
  double integrate_pixel(cctbx::miller::index<> h, std::size_t panel, int x,
                         int y) const {
    ...
    if (rlp.length() > 0) {
      ...
      mat3<double> sigma_lw =
          (sigma_wavelength_spread_ + sigma_angular_spread_) * rlp.length();
      ...
      mat3<double> E(e1[0], e2[0], e3[0], e1[1], e2[1], e3[1], e1[2], e2[2],
                     e3[2]);
    ...
    for (std::size_t j = 0; j < N; ++j) {
      for (std::size_t i = 0; i < N; ++i) {
        double xx = x + (i + 0.5) / N;
        double yy = y + (j + 0.5) / N;
        vec3<double> ss = detector_[panel]
                              .get_pixel_lab_coord(vec2<double>(xx, yy))
                              .normalize() *
                          s0_length;
