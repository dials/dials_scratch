...
def tst_covariance_matrix():
    ...
    cov.compose(
        reciprocal_lattice_point_spread=parameters, mosaic_block_angular_spread=0.01
    )

    assert all(
        abs(p1 - p2) < 1e-7
        for p1, p2 in zip(cov.reciprocal_lattice_point_spread().parameters, parameters)
    )
...
def tst_model_3d():
    ...
    for refl in reflections:
        ...
        cov.compose(
            reciprocal_lattice_point_spread=(0.001, 0.000, 0.001, 0.000, 0.000, 0.001)
        )
