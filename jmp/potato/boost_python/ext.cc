 ...
 */
mat3<double> compute_change_of_basis_operation(vec3<double> s0,
                                               vec3<double> s2) {
 ...
 * @return The change of basis matrix
 */
mat3<double> compute_change_of_basis_operation2(vec3<double> s0,
                                                vec3<double> r) {
...
public:
   ...
   * @param sigma The covariance matrix
   */
  PredictorAngular(Experiment experiment, mat3<double> sigma,
                   double probability)
      : PredictorBase(experiment, probability), sigma_(sigma) {}
...
protected:
   ...
   */
  int6 compute_single(vec3<double> s1, vec3<double> s2, std::size_t panel_id,
                      double D) const {
...
public:
   ...
   * @param sigma The covariance matrix
   */
  BBoxCalculatorSimple(Experiment experiment, mat3<double> sigma,
                       double probability, int border)
...
public:
   ...
   * @param sigma The covariance matrix
   */
  BBoxCalculatorAngular(Experiment experiment, mat3<double> sigma,
                        double probability, int border)
      : BBoxCalculatorBase(experiment, probability, border), sigma_(sigma) {}
...
protected:
   ...
   * @param D the distance
   */
  void compute_single(vec3<double> s1, vec3<double> s2, Shoebox<> sbox,
                      double D) const {
    ...
    for (std::size_t j = 0; j < mask.accessor()[1]; ++j) {
      for (std::size_t i = 0; i < mask.accessor()[2]; ++i) {
        ...
        vec3<double> sp1 =
            panel.get_pixel_lab_coord(p1).normalize() * s0_length;
        vec3<double> sp2 =
            panel.get_pixel_lab_coord(p2).normalize() * s0_length;
        vec3<double> sp3 =
            panel.get_pixel_lab_coord(p3).normalize() * s0_length;
        vec3<double> sp4 =
            panel.get_pixel_lab_coord(p4).normalize() * s0_length;
...
public:
   ...
   * @param sigma The covariance matrix
   */
  MaskCalculatorSimple(Experiment experiment, mat3<double> sigma,
                       double probability)
...
public:
   ...
   * @param sigma The covariance matrix
   */
  MaskCalculatorAngular(Experiment experiment, mat3<double> sigma,
                        double probability)
      : MaskCalculatorBase(experiment, probability), sigma_(sigma) {}
...
BOOST_PYTHON_MODULE(dials_scratch_jmp_potato_ext) {
  ...
  class_<BBoxCalculatorSimple, bases<BBoxCalculatorBase>>(
      "BBoxCalculatorSimple", no_init)
      .def(init<Experiment, mat3<double>, double, int>());

  class_<BBoxCalculatorAngular, bases<BBoxCalculatorBase>>(
      "BBoxCalculatorAngular", no_init)
      .def(init<Experiment, mat3<double>, double, int>());

  class_<MaskCalculatorBase>("MaskCalculatorBase", no_init)
      .def("compute", &MaskCalculatorBase::compute);

  class_<MaskCalculatorSimple, bases<MaskCalculatorBase>>(
      "MaskCalculatorSimple", no_init)
      .def(init<Experiment, mat3<double>, double>());

  class_<MaskCalculatorAngular, bases<MaskCalculatorBase>>(
      "MaskCalculatorAngular", no_init)
      .def(init<Experiment, mat3<double>, double>());
}
