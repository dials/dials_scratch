...
class IncidentBeamFactor(ScaleFactor):
  ...
  with other factors that can be modelled as a function of rotation angle"""
    ...
    def __init__(self, phi_range_deg, deg_per_interval=5):
        ...
        n_intervals = max(
            int(abs(phi_range_deg[1] - phi_range_deg[0]) / deg_per_interval), 1
        )
        ...
        value = 1
        self._param = ScanVaryingParameterSet(
            value, self._num_samples, name="IncidentBeam"
        )
  ...
  factors."""
    ...
    derivatives of that scale factor wrt all parameters of the model"""
        ...
        if len(scale_components) > 1:
            ...
            grad_components = [
                row_multiply(g, coeff)
                for g, coeff in zip(grad_components, omit_one_prods)
            ]
