...
def compute_mode(mu, sigma):
    ...
    # Compute the mode
    x = matrix.col((mu_prime[0] / (1 - l * L[0]), mu_prime[1] / (1 - l * L[3])))
...
def compute_mean(mu, sigma):
    ...
    x0 = peak[0]
    A = (
        sqrt(2 * pi / abs(1 / sigma[0]))
        * x0
        * exp(-0.5 * (x0 - mu[0]) ** 2 / sigma[0] ** 2)
    )
    B = sqrt(2 * pi / abs(1 / sigma[0])) * exp(-0.5 * (x0 - mu[0]) ** 2 / sigma[0] ** 2)
    XX = A / B
    y0 = peak[1]
    A = (
        sqrt(2 * pi / abs(1 / sigma[3] ** 2))
        * y0
        * exp(-0.5 * (y0 - mu[1]) ** 2 / sigma[3] ** 2)
    )
    B = sqrt(2 * pi / abs(1 / sigma[3] ** 2)) * exp(
        -0.5 * (y0 - mu[1]) ** 2 / sigma[3] ** 2
    )
    YY = A / B
