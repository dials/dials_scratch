...
class PutativeBijvoetPairFactory(object):
    def __init__(self, intensity_cutoff, distance_cutoff, radius_cutoff, beam_centre):
...
class Script(object):
    def __init__(self):
        ...
        warnmsg = (
            "WARNING: The {0} contains more than one "
            "{1}. Only the first will be considered."
        )
        if params.input.datablock:
            if params.input.experiments:
                raise Sorry(
                    "Please provide either a datablock or an experiment list, "
                    "not a combination of the two"
                )
            if len(params.input.datablock) != 1:
                raise Sorry(
                    "Please provide a single datablock or experiment list as input"
                )
        ...
        if "intensity.sum.value" not in self.reflections:
            raise Sorry(
                "The provided reflection table does not have a column "
        ...
        if len(self.detector) != 1:
            raise Sorry(
                "The detector has multiple panels, which is not currently "
        ...
        if params.beam_centre_convention == "mosflm":
            ...
            assert self.params.beam_centre_convention == "dials"
    ...
    def __call__(self):
        ...
        ax.set_title(
            (
                "Bisectors of lines joining potential Bijvoet mates.\n"
                "X and Y using the {0} convention"
            ).format(self.params.beam_centre_convention.upper())
        )
        ...
        for pair in pairs:
            ...
            ax.plot(
                [pair.bisector_x1, pair.bisector_x2],
