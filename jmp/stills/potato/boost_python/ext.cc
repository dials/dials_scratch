...
public:
  ...
  ConditionalDistributionAtZ(vec3<double> mu, mat3<double> sigma, double z) {
    ...
    // |   -------     ---   |
    // | | S31 S32 | | S33 | |
    mat2<double> sigma11(sigma[0], sigma[1], sigma[3], sigma[4]);
   ...
   */
  vec2<double> mu() const { return mu_; }

  /**
   * @returns The covariance of the conditional distribution
   */
  mat2<double> sigma() const { return sigma_; }
...
public:
  PotatoOnEwaldSphere(double wavelength, vec3<double> mu, mat3<double> sigma)
      : mu_(mu), sigma_(sigma), R_(compute_rotation_of_mu_onto_z(mu)),
        mup_(R_.transpose() * mu), sigmap_(R_.transpose() * sigma_ * R_) {
  ...
  double log_likelihood() const {
    ...
    return -0.5 *
           (std::log(sigma) + (1 / sigma) * (radius_ - mu) * (radius_ - mu));
  }
  ...
  double conditional_likelihood(double x, double y) const {
    ...
    return -0.5 * (std::log(sigma.determinant()) +
                   (x - mu) * sigma.inverse() * (x - mu));
  }
  ...
  vec3<double> centre_of_mass_on_ewald_sphere() const {
    ...
    vec3<double> v(distribution.mu()[0], distribution.mu()[1], radius_);
...
protected:
  ...
  mat3<double> compute_rotation_of_mu_onto_z(vec3<double> mu) const {
    ...
    if (angle < TINY) {
      return mat3<double>(1, 0, 0, 0, 1, 0, 0, 0, 1);
    }
...
BOOST_PYTHON_MODULE(dials_scratch_jmp_stills_potato_ext) {
  ...
  class_<PotatoOnEwaldSphere>("PotatoOnEwaldSphere", no_init)
      .def(init<double, vec3<double>, mat3<double>>())
      .def("scale_factor", &PotatoOnEwaldSphere::scale_factor)
      .def("centre_of_mass_on_ewald_sphere",
           &PotatoOnEwaldSphere::centre_of_mass_on_ewald_sphere)
      .def("log_likelihood", &PotatoOnEwaldSphere::log_likelihood)
      .def("conditional_mean", &PotatoOnEwaldSphere::conditional_mean)
      .def("conditional_sigma", &PotatoOnEwaldSphere::conditional_sigma)
