...
class Spec {
public:
  Spec(const Beam &beam, const Detector &detector, const Goniometer &goniometer,
       const Scan &scan, double delta_b, double delta_m)
      : beam_(beam), detector_(detector), goniometer_(goniometer), scan_(scan),
        delta_b_(delta_b), delta_m_(delta_m) {
  ...
  const Goniometer &goniometer() const { return goniometer_; }

  const Scan &scan() const { return scan_; }

  const double delta_b() const { return delta_b_; }

  const double delta_m() const { return delta_m_; }
 ...
 */
template <typename FloatType> class SingleProfileLearner {
public:
   ...
   */
  SingleProfileLearner(const Spec spec, int3 grid_size)
      : detector_(spec.detector()), scan_(spec.scan()),
        m2_(spec.goniometer().get_rotation_axis()), s0_(spec.beam().get_s0()),
        delta_b_(spec.delta_b()), delta_m_(spec.delta_m()),
        data_(af::c_grid<3>(2 * grid_size[0] + 1, 2 * grid_size[1] + 1,
                            2 * grid_size[2] + 1),
              0.0),
        finalized_(false), count_(0) {
   ...
   * @returns The profile.
   */
  profile_type data() const { return data_; }
   ...
   */
  profile_type get(std::size_t panel_number, const vec3<double> &s1, double phi,
                   int6 bbox) const {
 ...
 */
template <typename FloatType> class MultiExpSingleProfileLearner {
public:
  typedef FloatType float_type;
   ...
   */
  MultiExpSingleProfileLearner(af::const_ref<Spec> spec,
    ...
    for (std::size_t i = 0; i < spec.size(); ++i) {
      learner_.push_back(
          learner_type(spec[i], int3(grid_size, grid_size, grid_size)));
    }
   ...
   */
  profile_type get(std::size_t id, std::size_t panel, const vec3<double> &s1,
                   double phi, int6 bbox) const {
   ...
   */
  std::size_t size() const { return learner_.size(); }
...
public:
  ...
  ImageSpaceProfileFitting(std::size_t grid_size) : grid_size_(grid_size) {
    DIALS_ASSERT(grid_size > 0);
  }
   ...
   */
  void add(const Spec &spec) { spec_.push_back(spec); }

  /**
   * Perform the profile fitting on the input data
   * @param data The reflection data
   */
  reference_learner_type execute(af::reflection_table data) const {
    ...
    for (std::size_t i = 0; i < data.size(); ++i) {
      ...
      if (!(flags[i] & DontIntegrate)) {
        ...
        profile_type profile =
            reference.get(id[i], sbox.panel, s1[i], xyzcal[i][2], sbox.bbox);
        ...
        if (mask_count > 0) {
          fitting_type fit(profile.const_ref(), mask.const_ref(),
                           sbox.data.const_ref(), sbox.background.const_ref());
