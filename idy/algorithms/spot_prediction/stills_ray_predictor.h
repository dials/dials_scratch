...
/**
 *  Parent class StillsRayPredictor always assumes that the center of impact is
 * at radius r from the origin of reciprocal space
 *
  ...
  Ray operator()(miller_index h, mat3<double> ub) {
    ...
    if (std::abs(delpsi_) > half_mosaicity_rad_) {
      ...
      vec3<double> r_new = q.unit_rotate_around_origin(
          ...
          e1, (delpsi_ > 0.) ? 10 * half_mosaicity_rad_
                             : -10 * half_mosaicity_rad_);
      s1 = (s0_ + r_new).normalize() * s0_.length();
    }
    return Ray(s1, 0.0, false);
  }
  double get_delpsi() const { return delpsi_; }
