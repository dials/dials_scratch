...
def fit(data, mask, model_data, model_mask, kernel):
    ...
    scaled_data.set_selected(
        indices, summed_data.select(indices).as_double() / summed_model.select(indices)
    )
...
if __name__ == "__main__":
  ...
  """
    ...
    parser = OptionParser(phil=phil_scope, read_experiments=True, read_reflections=True)
    ...
    reflections.unset_flags(
        flex.size_t(range(len(reflections))), reflections.flags.integrated_sum
    )
    reflections.unset_flags(
        flex.size_t(range(len(reflections))), reflections.flags.integrated_prf
    )
    ...
    for frame in range(0, len(imageset)):
        ...
        sbox_mask = reflections.select(indices)["shoebox"].apply_background_mask(
        ...
        threshold = DispersionThreshold(data.all(), (3, 3), 6, 3, 0, 2)
        ...
        reflections["shoebox"].select(indices).apply_pixel_data(
            data.as_double(), background, raw_mask, frame, 1
        )
