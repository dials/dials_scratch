...
class ReflectionFrame(wx.Frame):
    def __init__(self, *args, **kwargs):
        ...
        btn_nxt_refl = wx.lib.buttons.GenBitmapTextButton(
            self, label=lbl_ref, bitmap=img_tmp
        )
        img_tmp = wx.ArtProvider.GetBitmap(wx.ART_GO_BACK, wx.ART_OTHER, (16, 16))
        btn_prv_refl = wx.lib.buttons.GenBitmapTextButton(
            self, label=lbl_ref, bitmap=img_tmp
        )
        ...
        img_tmp = wx.ArtProvider.GetBitmap(wx.ART_GO_FORWARD, wx.ART_OTHER, (16, 16))
        btn_nxt_slice = wx.lib.buttons.GenBitmapTextButton(
            self, label="slice", bitmap=img_tmp
        )

        img_tmp = wx.ArtProvider.GetBitmap(wx.ART_GO_BACK, wx.ART_OTHER, (16, 16))
        btn_prv_slice = wx.lib.buttons.GenBitmapTextButton(
            self, label="slice", bitmap=img_tmp
        )
        ...
        for indx in range(len(self.Image)):
            self.Image[indx] = wx.StaticBitmap(
        ...
        div_h_box = wx.BoxSizer(wx.HORIZONTAL)
        div_h_box.Add(
            self.Image[0], 0, wx.ALIGN_CENTER_HORIZONTAL | wx.ALL | wx.ADJUST_MINSIZE, 5
        )

        div_h_box.Add(
            self.Image[1], 0, wx.ALIGN_CENTER_HORIZONTAL | wx.ALL | wx.ADJUST_MINSIZE, 5
        )

        div_h_box.Add(
            self.Image[2], 0, wx.ALIGN_CENTER_HORIZONTAL | wx.ALL | wx.ADJUST_MINSIZE, 5
        )
    ...
    def _My_Update(self, request_new_data=True):
        ...
        if request_new_data == True:
            ...
            for indx in range(len(self.arr_img)):
                ...
                self.wx_Img[indx] = self.bmp(
                    np_img_2d=self.arr_img[indx],
                    Intst_max=Imax,
                    ofst=self.bbox_px,
                    xyz=self.xyz_px,
                    title=self.arr_title[indx],
        ...
        for indx in range(len(self.arr_img)):
            if self.arr_img[indx] == None:
                ...
                empty_flag = False
            self.My_Img[indx] = self.bmp.from_wx_image_to_wx_bitmap(
                self.wx_Img[indx], self.frame_scale, empty=empty_flag
            )
