...
def run(args):
    ...
    intensities = [ma for ma in as_miller_arrays if ma.info().labels == ["I", "SIGI"]][
        0
    ]
    intensities = intensities.customized_copy(
        indices=mtz_object.extract_original_index_miller_indices()
    ).set_info(intensities.info())
    batch_ids = [ma for ma in as_miller_arrays if ma.info().labels == ["BATCH"]][0]
    batch_ids = batch_ids.customized_copy(
        indices=mtz_object.extract_original_index_miller_indices()
    ).set_info(batch_ids.info())
    ...
    for negate in (False, True):
        ...
        intensities_merged = (
            intensities_sel.customized_copy(anomalous_flag=False)
            .merge_equivalents()
            .array()
        )
