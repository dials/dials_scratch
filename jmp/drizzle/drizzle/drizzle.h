...
void drizzle(af::ref<double, af::c_grid<2>> dst,
             ...
             double fraction, Transform transform) {
  ...
  for (std::size_t j = 0; j < src.accessor()[0]; ++j) {
    for (std::size_t i = 0; i < src.accessor()[1]; ++i) {
      ...
      for (int y = y0; y < y1; ++y) {
        for (int x = x0; x < x1; ++x) {
          vert4 b(vec2<double>(x, y), vec2<double>(x + 1, y),
                  vec2<double>(x + 1, y + 1),
                  vec2<double>(x, y + 1)) double frac_area =
              simple_area(quad_with_convex_quad(a, b));
          double frac_weight = frac_area * src_weight(j, i);
          double weight = frac_weight + dst_weight(y, x);
          if (weight != 0) {
            double value =
                frac_weight * src(j, i) + dst(y, x) * dst_weight(y, x);
            dst(y, x) = num / weight;
          }
