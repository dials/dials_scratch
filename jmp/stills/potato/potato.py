...
from dials_scratch.jmp.stills.potato.profile_refiner import (
...
from dials_scratch.jmp.stills.potato import chisq_quantile
    ...
    def __init__(self, experiment, parameters, dmin=None):
        ...
        for h in miller_indices_to_test:
            ...
            if d < quantile:
                ...
                v = (
                    matrix.col((mubar[0], mubar[1], s0.length())).normalize()
                    * s0.length()
                )
                s1 = R.transpose() * v
...
class Integrator(object):
    ...
    def _compute_partiality(self):
        ...
        for k in range(len(self.reflections)):
            ...
            mu2 = mu[2]
            partiality[k] = exp(
                -0.5 * (s0.length() - mu2) * (1 / S22) * (s0.length() - mu2)
            )
    ...
    def _refine_profile(self):
        ...
        if self._profile_parameters is None:
            self._profile_parameters = matrix.col(
                (self.sigma_d, 0, self.sigma_d, 0, 0, self.sigma_d)
            )
        refiner = ProfileRefiner(self._refiner_data, self._profile_parameters)
        refiner.refine()
    ...
    def _refine_crystal(self):
        ...
        refiner = CrystalRefiner(
            self.experiments[0], self.reflections, self._profile_parameters
        )
