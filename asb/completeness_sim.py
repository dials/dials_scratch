...
if img_path is not None and os.path.exists(img_path):
    ...
    detector = detector_factory.simple(
        "SENSOR_UNKNOWN", 100, (97.075, 97.075), "+x", "-y", (0.11, 0.11), (1765, 1765)
    )
...
def get_random_predictions():
    ...
    expt = Experiment(
        beam=beam, detector=detector, goniometer=None, scan=None, crystal=cm
    )
...
while True:
    ...
    if "miller_set" not in locals():
        miller_set = miller.set(
            known_symmetry, predictions["miller_index"], anomalous_flag=True
        )
    else:
    ...
    ccano = 0.36
    print(
        "Estimated Sanom assuming CCano %.2f, %d site and f=%.2f: %.3f"
        % (ccano, n_sites, f, 0.36 * sqrt(n_obs) / (sqrt(1) * sqrt(2)))
    )

    # Thresholding
    if [binned_data.data[i] < 95 for i in binned_data.binner.range_used()].count(
        False
    ) != 10:
        continue

    if [
        binner.counts()[i] / binner.counts_complete()[i] < 4
