...
def find_ext_mask_3d(diffdata3d):
    ...
    ext_area = 1
    diffdata3d_ext = numpy.zeros(n_row * n_col * n_frm, dtype=int).reshape(
        n_frm, n_row, n_col
    )
        ...
        for row in range(ext_area, n_row - ext_area + 1, 1):
            for col in range(ext_area, n_col - ext_area + 1, 1):
                if diffdata3d[frm, row, col] == 1:
                    diffdata3d_ext[
                        frm - ext_area : frm + ext_area + 1,
                        row - ext_area : row + ext_area + 1,
                        col - ext_area : col + ext_area + 1,
                    ] = 1
        ...
        for col in range(0, n_col, 1):
            if mask[row, col] == 1 and tmp_mask[row, col] == 1:
                ...
                while in_img == "True":
                    ...
                    if (
                        bot_bound == bot_old
                        and top_bound == top_old
                        and lft_bound == lft_old
                        and rgt_bound == rgt_old
                    ):
                        break
                    bot_old = bot_bound
                    top_old = top_bound
...
def find_bound_3d(diffdata3d):
    ...
    tmp_3d_mask = numpy.zeros(n_row * n_col * n_frm, dtype=int).reshape(
        n_frm, n_row, n_col
    )
                ...
                if tmp_3d_mask[frm, row, col] == 1:
                    ...
                    while in_img == "True":
                        ...
                        if (
                            ...
                            and lft_bound == lft_old
                            and rgt_bound == rgt_old
                            and bck_bound == bck_old
                            and frn_bound == frn_old
                        ):
                            break
                        bot_old = bot_bound
                        top_old = top_bound
