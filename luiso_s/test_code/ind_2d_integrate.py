    ...
    for pos in range(len(xcoord)):
        ...
        for times in range(5):
            for x in range(from_x, x_to, 1):
                for y in range(from_y, y_to, 1):
                    pscan = float(
                        numpy.sum(data2dtmp[x - 1 : x + 2, y - 1 : y + 2]) / 9.0
                    )
                    data2dsmoth[x, y] = pscan
            data2dtmp = data2dsmoth

        data2dsmoth[from_x:x_to, from_y:y_to] = (
            data2dsmoth[from_x:x_to, from_y:y_to] + threshold_shift
        )
        ...
        for x in range(from_x, x_to, 1):
            for y in range(from_y, y_to, 1):
                if diffdata2d[x, y] == 1:
                    diffdata2d_ext[
                        x - ext_area : x + ext_area + 1, y - ext_area : y + ext_area + 1
                    ] = 1
        ...
        for x in range(from_x, x_to, 1):
            for y in range(from_y, y_to, 1):
                if diffdata2d_ext[x, y] == 1:
                    x_num_sum = (
                        x_num_sum
                        + float(data2d[x, y]) * (float(x) - float(x_cm[pos])) ** 2.0
                    )
                    y_num_sum = (
                        y_num_sum
                        + float(data2d[x, y]) * (float(y) - float(y_cm[pos])) ** 2.0
                    )
                    den_sum = den_sum + float(data2d[x, y])
        if den_sum > 0.0:
