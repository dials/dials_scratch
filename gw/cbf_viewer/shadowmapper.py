...
class Shadow(object):
    ...
    def orient_gonio(self, kappa=-180.0, omega=90.0):
        ...
        r_om = np.array([[1.0, 0.0, 0.0], [0.0, cosom, sinom], [0.0, -sinom, cosom]])
    ...
    def shadow_texture(self, kappa=-180.0, omega=90.0, blank=False):
        ...
        for i in range(len(dist)):
            dist[i] = np.dot(self.origin[0], normal) / np.dot(new_coords[i], normal)
            proj[i] = dist[i] * new_coords[i]

            x_det2[i] = (
                -np.sign(self.origin[0, 0] - proj[i, 0])
                * (
                    norm(np.cross(self.slow[0], (self.origin[0] - proj[i])))
                    / norm(self.slow[0])
                )
                / pix[0]
            )
            y_det2[i] = (
                np.sign(self.origin[0, 1] - proj[i, 1])
                * (
                    norm(np.cross(self.fast[0], (self.origin[0] - proj[i])))
                    / norm(self.fast[0])
                )
                / pix[1]
            )
    ...
    def affected_pixels(self, kappa=-180.0, omega=90.0, blank=False):
        ...
        (
            ...
            tex_noshad,
            noshad_data,
        ) = self.shadow_texture(kappa, omega, blank)
    ...
    def pixel_plotter(self, blank=True, num_points=50):
        ...
        pixels = np.array(
            [
                self.affected_pixels(x, y, blank=blank)[1]
                for x, y in zip(np.ravel(X), np.ravel(Y))
            ]
        ) / (
            viewer.dimensions[0][0]
            * viewer.dimensions[0][1]
