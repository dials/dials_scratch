...
void test(const Beam &beam, const Detector &detector,
          const Goniometer &goniometer, const Scan &scan, double sigma_b,
          double sigma_m, std::size_t grid_size, af::reflection_table data) {
  ...
  TransformSpec<> spec(beam, detector, goniometer, scan, sigma_b, sigma_m, 5.0,
                       grid_size);
...
#pragma omp parallel for
  for (std::size_t i = 0; i < s1.size(); ++i) {
    ...
    if (bbox[0] < 0 || bbox[1] > image_size[0] || bbox[2] < 0 ||
        bbox[3] > image_size[1]) {
      continue;
    }

    Forward<> transform(spec, s1[i], xyzmm[i][2], sbox[i]);
  }
}
