...
def start(data2d, xcoord, ycoord, cntrd_xcoord, cntrd_ycoord, x_sigma, y_sigma):
        ...
        for x_scan in range(int(xcoord[pos]) - 5, int(xcoord[pos] + 6), 1):
            for y_scan in range(int(ycoord[pos]) - 5, int(ycoord[pos]) + 6, 1):
                if diffdata2d_ext[x_scan, y_scan] == 1:
                    x_num_sum = x_num_sum + float(data2d[x_scan, y_scan]) * float(
                        x_scan
                    )
                    y_num_sum = y_num_sum + float(data2d[x_scan, y_scan]) * float(
                        y_scan
                    )
        ...
        for x_scan in range(int(xcoord[pos]) - 5, int(xcoord[pos]) + 6, 1):
            for y_scan in range(int(ycoord[pos]) - 5, int(ycoord[pos]) + 6, 1):
                if diffdata2d_ext[x_scan, y_scan] == 1:
                    x_num_sum = (
                        x_num_sum
                        + float(data2d[x_scan, y_scan])
                        * (float(x_scan) - float(cntrd_xcoord[pos])) ** 2.0
                    )
                    y_num_sum = (
                        y_num_sum
                        + float(data2d[x_scan, y_scan])
                        * (float(y_scan) - float(cntrd_ycoord[pos])) ** 2.0
                    )
                    den_sum = den_sum + float(data2d[x_scan, y_scan])
        if den_sum > 0.0:
    ...
    for pos in range(len(xcoord)):
        paintmask[
            xcoord[pos] - 5 : xcoord[pos] + 6, ycoord[pos] - 5 : ycoord[pos] + 6
        ] = (
            paintmask[
                xcoord[pos] - 5 : xcoord[pos] + 6, ycoord[pos] - 5 : ycoord[pos] + 6
            ]
        ...
        nint_xcoord = int(xcoord[pos] + 0.5)
        nint_ycoord = int(ycoord[pos] + 0.5)
