...
if __name__ == "__main__":
    ...
    for r in rlist:
        ...
        background_pixels = r.shoebox.select(
            flex.bool([bool(p & MaskCode.BackgroundUsed) for p in r.shoebox_mask])
        )

        # background_pixels = r.shoebox.as_1d()

        signal_pixels = r.shoebox.select(
            flex.bool([bool(p & MaskCode.Foreground) for p in r.shoebox_mask])
        )

        m = len(signal_pixels)
        n = len(background_pixels)
        Ib = m * flex.mean(
            r.shoebox_background
        )  # (m / n) * flex.sum(background_pixels)
