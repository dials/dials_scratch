...
class Script(object):
    ...
    def run(self):
        ...
        self.original_reflections = flatten_reflections(self.params.input.reflections)[
            0
        ]

        print(
            "Number of reflections loaded: {0}".format(len(self.original_reflections))
        )
    ...
    def set_random_error(self):
        ...
        self.shiftX_px = flex.double(
            [random.gauss(0, self.params.centroid_sigmas.sigX_px) for _ in xrange(nref)]
        )
        self.shiftY_px = flex.double(
            [random.gauss(0, self.params.centroid_sigmas.sigY_px) for _ in xrange(nref)]
        )
        self.shiftZ_px = flex.double(
            [random.gauss(0, self.params.centroid_sigmas.sigZ_px) for _ in xrange(nref)]
        )
        return
    ...
    def create_indexed(self, experiments, filename):
        ...
        # new 'observations'
        shift_px = flex.vec3_double(self.shiftX_px, self.shiftY_px, self.shiftZ_px)
