...
#for(i in 3:200){
#  dat <- rnorm(i)
#  a <- pgram(dat, detrend=F, demean=T, taper=0, plot=F, fast=F)$spec
#  b <- abs(fft(dat - mean(dat)))^2 / length(dat)
#  b <- b[2:(1+length(a))]
#  print(max(abs(a-b)))
#}

pgram <- function (x, spans = NULL, kernel = NULL, taper = 0.1, pad = 0,
    ...
    na.action = na.fail, ...)
{
    ...
    cat('N is\n')
    print(N)
    cat('pgram before fiddling with end values\n')
    print(pgram[,1,1])
    for (i in 1L:ncol(x)) {
        for (j in 1L:ncol(x)) {
    ...
    if (!is.null(kernel)) {
        cat('pgram before kernapply\n')
        print(pgram[,1,1])
        cat('kernapply(pgram) taking the real part only:')
        print(kernapply(Re(pgram[, 1, 1]), kernel, circular = TRUE))
        for (i in 1L:ncol(x)) for (j in 1L:ncol(x)) pgram[, i,
            j] <- kernapply(pgram[, i, j], kernel, circular = TRUE)
        df <- df.kernel(kernel)
        bandwidth <- bandwidth.kernel(kernel)
        cat("After kernapply, pgram is:\n")
        print(pgram[,1,1])
    }
    else {
        df <- 2
