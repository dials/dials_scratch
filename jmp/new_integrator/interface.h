...
public:
   ...
   */
  ShoeboxExtractor(af::reflection_table data, std::size_t npanels, int frame0,
                   int frame1)
      : npanels_(npanels), frame0_(frame0), frame1_(frame1) {
...
public:
   ...
   */
  IntegrationTask3DMultiExecutor(af::reflection_table data, tiny<int, 2> job,
                                 std::size_t npanels)
      : data_(data), frame0_(job[0]), frame1_(job[1]), frame_(frame0_),
        nframes_(frame1_ - frame0_), npanels_(npanels) {
  ...
  int frame0() const { return frame0_; }

  /** @returns The last frame */
  int frame1() const { return frame1_; }
  ...
  std::size_t npanels() const { return npanels_; }
   ...
   */
  bool finished() const { return frame_ == frame1_; }
...
public:
   ...
   */
  JobManager(const af::const_ref<std::size_t> &panel,
             const af::const_ref<int6> &bbox,
             const af::const_ref<std::size_t> &flags,
             const af::const_ref<tiny<int, 2>> &jobs, std::size_t npanels)
      : jobs_(jobs.begin(), jobs.end()), npanels_(npanels),
        ndata_(panel.size()) {
   ...
   */
  std::size_t njobs() const { return jobs_.size(); }

  /**
   * @returns The number of data items
   */
  std::size_t ndata() const { return ndata_; }

private:
  /**
   * Compute the indices of the reflections in each job.
   */
  void compute_indices(const af::const_ref<std::size_t> &panel,
                       const af::const_ref<int6> &bbox,
                       const af::const_ref<std::size_t> &flags) {
...
public:
   ...
   */
  IntegrationTask3DExecutor(af::reflection_table data,
                            const af::const_ref<tiny<int, 2>> &jobs,
                            std::size_t npanels, callback_type callback)
      : data_(data),
        job_manager_(data["panel"], data["bbox"], data["flags"], jobs, npanels),
        allocator_(job_manager_), process_(callback) {
   ...
   * @returns The first frame.
   */
  int frame0() const { return job_manager_.frame0(); }

  /**
   * @returns The last frame
   */
  int frame1() const { return job_manager_.frame1(); }
   ...
   * @returns The number of frames
   */
  std::size_t nframes() const { return job_manager_.nframes(); }

  /**
   * @returns The number of panels
   */
  std::size_t npanels() const { return job_manager_.npanels(); }
   ...
   */
  bool finished() const { return frame_ == frame1(); }
...
private:
  ...
  af::reflection_table split(std::size_t job) const {
    ...
    af::reflection_table result =
        select_rows_index(data_, job_manager_.indices(job));
  ...
  void merge(af::reflection_table result, std::size_t job) {
    using namespace dials::af::boost_python::flex_table_suite;
    set_selected_rows_index_mask(data_, job_manager_.indices(job),
                                 job_manager_.mask(job), result);
  }
 ...
 */
af::shared<tiny<int, 2>> compute_jobs(tiny<int, 2> array_range,
                                      double block_size) {
...
public:
   ...
   * @param npanels The number of panels
   */
  IntegrationManager3DExecutor(af::reflection_table data, vec2<int> array_range,
                               double block_size)
      : data_(data), array_range_(array_range), block_size_(block_size),
        finished_(false) {
...
public:
  IntegrationManager3DMultiExecutor(af::reflection_table reflections,
                                    vec2<int> array_range, double block_size)
      : data_(reflections) {
   ...
   */
  bool finished() const { return finished_.all_eq(true); }

  /**
   * @returns The number of tasks
   */
  std::size_t size() const { return finished_.size(); }
   ...
   */
  af::shared<std::size_t> ignored() const {
    return af::shared<std::size_t>(&ignored_[0],
                                   &ignored_[0] + ignored_.size());
  }
