...
public:
  ...
  GroupedObservations(af::const_ref<miller_index> group_index,
                      af::shared<double> intensity, af::shared<double> weight,
                      af::shared<double> phi, af::shared<double> scale)
      : group_index_(group_index.begin(),
                     group_index.end()), // initialisation lists
        intensity_(intensity), weight_(weight), phi_(phi), scale_(scale) {
  ...
  // intensity accessors
  af::shared<double> get_intensity() const { return intensity_; }
  ...
  // weight accessors
  af::shared<double> get_weight() const { return weight_; }
  ...
  // scale accessors
  af::shared<double> get_scale() const { return scale_; }
  ...
  // phi getter
  af::shared<double> get_phi() const { return phi_; }

  // group size getter
  af::shared<std::size_t> get_group_size() const { return group_size_; }
