...
#    quit()
filenames = [
    "/scratch/my_code/dials/dials_data/data_from__dls__i02__data__2013__nt5964-1__2013_02_08_GW__DNA__P1__X4/X4_lots_M1S4_1_0001.cbf",
...
data3d = numpy.zeros((row_dif) * (col_dif) * n_frm, dtype=int).reshape(
    n_frm, row_dif, col_dif
)
...
dif3d = numpy.zeros((row_dif) * (col_dif) * n_frm, dtype=int).reshape(
    n_frm, row_dif, col_dif
)
smoth3d = numpy.zeros((row_dif) * (col_dif) * n_frm, dtype=int).reshape(
    n_frm, row_dif, col_dif
)
time1 = time.time()
...
x_from_lst, x_to_lst, y_from_lst, y_to_lst, z_from_lst, z_to_lst = find_bound_3d(
    dif_3d_ext
)
time4 = time.time()
...
for pos in range(len(x_from_lst)):
    print(
        "x_from_lst, x_to_lst, y_from_lst, y_to_lst, z_from_lst, z_to_lst =",
        x_from_lst[pos],
        x_to_lst[pos],
        y_from_lst[pos],
        y_to_lst[pos],
...
for frm_tmp in range(n_frm):
    plt = pylab.imshow(
        dif3d[frm_tmp, :, :], cmap=cm.Greys_r, interpolation="nearest", origin="lower"
    )
    ...
    pylab.show()
    plt = pylab.imshow(
        data3d[frm_tmp, :, :], cmap=cm.Greys_r, interpolation="nearest", origin="lower"
    )
    pylab.show()
    plt = pylab.imshow(
        smoth3d[frm_tmp, :, :], cmap=cm.Greys_r, interpolation="nearest", origin="lower"
    )
    pylab.show()
