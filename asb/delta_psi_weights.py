...
class Script(object):
  ...
  def run(self):
    ...
    self.params = params
    assert len(params.input.experiments) == len(params.input.reflections) == 1, "Provide one experiment list and one reflection table"
    ...
    for expt_id, experiment in enumerate(experiments):
      ...
      if space_group is None:
        ...
        assert space_group.type().lookup_symbol() == experiment.crystal.get_space_group().type().lookup_symbol()
