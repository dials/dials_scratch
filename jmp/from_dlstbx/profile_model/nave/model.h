...
template <typename T> T min6(T a, T b, T c, T d, T e, T f) {
...
template <typename T> T max6(T a, T b, T c, T d, T e, T f) {
...
public:
  ...
  EwaldCirclePlaneProjection(vec3<double> s0, mat3<double> d)
      : s0_(s0), d0_(d[0], d[3], d[6]), d1_(d[1], d[4], d[7]),
        d2_(d[2], d[5], d[8]), d0s0_(d0_ * s0_), d1s0_(d1_ * s0_),
        d2s0_(d2_ * s0_), d0d2_(d0_ * d2_), d1d2_(d1_ * d2_),
        d2d2_(d2_.length_sq()) {}
...
public:
   ...
   */
  Model(vec3<double> s0, vec3<double> m2, vec3<double> s1, double phi, double s,
        double da, double w)
      : s0_(s0), m2_(m2.normalize()), s1_(s1.normalize() * s0.length()),
        e1_(s1_.cross(s0_).normalize()), e2_(s1_.cross(e1_).normalize()),
        e3_((s1_ + s0_).normalize()), zeta_(m2_ * e1_), phi_(phi), s_(s),
        da_(da), w_(w), cap_(s1_ - s0_, w_) {
    ...
    rocking_width_ = w + 2.0 * std::atan2(0.5 / s, r().length()) +
   ...
   */
  vec3<double> m2() const { return m2_; }

  /**
   * @returns The e1 axis
   */
  vec3<double> e1() const { return e1_; }

  /**
   * @returns The e2 axis
   */
  vec3<double> e2() const { return e2_; }

  /**
   * @returns The e3 axis
   */
  vec3<double> e3() const { return e3_; }
   ...
   */
  vec3<double> r() const { return cap_.axis(); }
   ...
   */
  double rocking_width() const { return rocking_width_; }

  /**
   * @returns The distance of the point from the profile.
   */
  double distance(vec3<double> r) const { return cap_.distance(r); }

  /**
   * @returns Is the point inside the profile bounds
   */
  bool inside(vec3<double> r) const { return distance(r) < thickness(); }
   ...
   */
  double z0() const { return cap_.h2() - thickness(); }

  /**
   * @returns The longest distance along the axis
   */
  double z1() const { return cap_.radius() + thickness(); }
  ...
  double intensity_fraction(double phia, double phib) const {
    ...
    double c =
        std::abs(zeta_) / (std::sqrt(2.0) * (0.5 * rocking_width_ / 3.0));
  ...
  vec2<double> ewald_intersection_angles() const {
    double rl = r().length();
    vec2<double> angles(ewald_intersection_angle(rl - thickness() / 2.0),
  ...
  af::small<vec3<double>, 8> minimum_box() const {
    ...
    vec3<double> v1(r * std::sin(theta1) * std::cos(phi1),
                    r * std::sin(theta1) * std::sin(phi1),
                    r * std::cos(theta1));
    vec3<double> v2(r * std::sin(theta1) * std::cos(phi2),
                    r * std::sin(theta1) * std::sin(phi2),
                    r * std::cos(theta1));
    vec3<double> v3(r * std::sin(theta1) * std::cos(phi3),
                    r * std::sin(theta1) * std::sin(phi3),
                    r * std::cos(theta1));

    // Compute the cartesian coordinates at the different extrema of r_max
    vec3<double> v4(r * std::sin(theta2) * std::cos(phi1),
                    r * std::sin(theta2) * std::sin(phi1),
                    r * std::cos(theta2));
    vec3<double> v5(r * std::sin(theta2) * std::cos(phi2),
                    r * std::sin(theta2) * std::sin(phi2),
                    r * std::cos(theta2));
    vec3<double> v6(r * std::sin(theta2) * std::cos(phi3),
  ...
  bool inside2(vec3<double> s, double angle0, double angle1) const {
    ...
    vec3<double> rr3 =
        rr.unit_rotate_around_origin(m2_, (angle0 + angle1) / 2.0 - phi_);
