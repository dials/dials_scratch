...
def refine_wavelengths(
    ...
    if initial_mosaic_parameters is None:
        domain_size_ang = flex.mean(
            flex.double([expt.crystal.get_domain_size_ang() for expt in experiments])
        )
        half_mosaicity_deg = flex.mean(
            flex.double([expt.crystal.get_half_mosaicity_deg() for expt in experiments])
        )
        ...
        def target(self, vector):
            ...
            if self.refine_bandpass:
                for i in xrange(len(self.experiments)):
                    ...
                    self.experiments[i].crystal.bandpass = (
                        midpoint - width,
                        midpoint + width,
                    )
                    if i == 0:
                        print(
                            "Experiment 0 bandpass:",
                            self.experiments[i].crystal.bandpass,
                        )

            # recompute the wavelengths
            self.reflections = self.wavelength_func(
                self.experiments, self.reflections, vector[0:2]
            )
            ...
            self.reflections = predictions_from_per_reflection_energies(
                experiments, self.reflections, tag, dest
            )
            ...
            rmsd = math.sqrt(
                flex.sum(
                    (
                        self.reflections["xyzobs.mm.value"]
                        - self.reflections["xyzcal.mm.%s" % dest]
                    ).norms()
                    ** 2
                )
    ...
    minimizer = simplex_minimizer(
        experiments,
        reflections,
        initial_mosaic_parameters,
        wavelength_func=wavelength_func,
        refine_bandpass=refine_bandpass,
    )
    final_mosaic_parameters = minimizer.x[0:2]
    print(
        "Final mosaic parameters",
        list(final_mosaic_parameters),
        experiments[0].crystal.bandpass,
    )
    if refine_bandpass:
    ...
    reflections = predictions_from_per_reflection_energies(
        experiments, minimizer.reflections, tag, dest
    )
    return reflections
...
def tophat_vector_wavelengths(experiments, reflections, mosaic_parameters):
    ...
    for expt_id, expt in enumerate(experiments):
        ...
        q = (
            flex.mat3_double(len(refls), expt.crystal.get_A())
            * refls["miller_index"].as_vec3_double()
        )
        e1 = q.cross(unit_s0).each_normalize()

        # length of an arc l = 2pir * angle/2pi = r*angle. So angle = l/r
        combined_mosaic_angle_approximation = ((2 / domain_size_ang) / q.norms()) + (
            half_mosaicity_deg * math.pi / 180
        )
        ...
        q_wavelength_min = (
            unit_s0.rotate_around_origin(e1, math.pi + z_wavelength_min) * q.norms()
        )
        q_wavelength_max = (
            unit_s0.rotate_around_origin(e1, math.pi + z_wavelength_max) * q.norms()
        )
...
def wavelengths_from_gaussians(experiments, reflections, mosaic_parameters):
    ...
    assert (new_wavelengths <= 0).count(True) == 0
    reflections[
        "reflection_wavelength_from_gaussian_mosaicity_and_bandpass"
    ] = new_wavelengths
