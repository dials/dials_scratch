...
void ReflectionList::setup(miller_index_list_type indices,
...
miller_index_list_type ReflectionList::get_indices() { return unique_indices_; }
...
void ReflectionList::set_unit_cell(
    scitbx::af::tiny<double, 6> unit_cell_params) {
...
void ReflectionList::setup_resolution_shells(size_t n_shells) {
  shells_.clear();
  high_limits_.clear();
  low_limits_.clear();
...
miller_index_list_type ReflectionList::get_shell(size_t shell) {
...
scitbx::af::shared<float> ReflectionList::shell_high_limits() {
  return high_limits_;
}
...
scitbx::af::shared<float> ReflectionList::shell_low_limits() {
  return low_limits_;
}
...
float ReflectionList::total_i_sigma() {
  CCTBX_ASSERT(reflections_.size() > 0);
...
scitbx::af::shared<float> ReflectionList::i_sigma_shells() {
  CCTBX_ASSERT(reflections_.size() > 0);
...
scitbx::af::shared<float> ReflectionList::total_i_sigma_shells() {
  CCTBX_ASSERT(reflections_.size() > 0);
  CCTBX_ASSERT(shells_.size() > 0);
...
scitbx::af::shared<float> ReflectionList::rmerge_shells() {
  CCTBX_ASSERT(reflections_.size() > 0);
  CCTBX_ASSERT(shells_.size() > 0);
