...
if __name__ == "__main__":
    ...
    for i in range(ncounts):
        ...
        mosuc_xyz = multivariate_normal(
            [0.0, 0.0, 0.0],
            [[mosuc_sigma, 0.0, 0.0], [0.0, mosuc_sigma, 0.0], [0.0, 0.0, mosuc_sigma]],
        )
        ...
        # phi += mosangle[1]
        r = matrix.col(
            (b * sin(theta) * cos(phi), b * sin(theta) * sin(phi), b * cos(theta))
        )
        ...
        # phi += divangle[1]
        o = matrix.col(
            (a * sin(theta) * cos(phi), a * sin(theta) * sin(phi), a * cos(theta))
        )
