...
from dials.command_line.cosym import phil_scope
...
def run(args=None):
    ...
    def array_from_refl_expts(refls, expts):
        ...
        cb_ops = change_of_basis_ops_to_minimum_cell(
            ExperimentList([expt]), 5.0, relative_length_tolerance= 0.05, absolute_angle_tolerance= 2.0,
        )
        ...
        arr = filtered_arrays_from_experiments_reflections(
            [expt], [refls], outlier_rejection_after_filter=False, partiality_threshold=0.4
        )
        return arr

    for refl_obj, expt_obj in zip(params.input.reflections, params.input.experiments):
        refls = refl_obj.data
        expts = expt_obj.data
    ...
    if subgroup:
        acentric_sg = subgroup["best_subsym"].space_group().build_derived_acentric_group()
    ...
    for i, (cb_op, dataset_id) in enumerate(zip(reindexing_ops, unique_ids)):
        ...
        expts = params.input.experiments[dataset_id].data
        refls = params.input.reflections[dataset_id].data
        for expt in expts:
        ...
        expts.as_file(f"symmetrized_{i}.expt")
        refls.as_file(f"symmetrized_{i}.refl")
