...
def extract_strong(directory):
  ...
  shutil.copyfile(os.path.join(directory, "integrated_experiments.json"), "experiments.json")
...
def generate_profile(sample=None, grid_size=15, grid_range=0.25):
  ...
  if sample is not None:
    selection = flex.size_t(random.sample(flex.size_t(range(len(reflections))), sample))
  ...
  if crystal.num_scan_points > 0:
    ...
    A = dict((z, crystal.get_A_at_scan_point(i)) for i, z in enumerate(range(*image_range)))
  else:
            ...
            for kk in range(5):
              for jj in range(5):
                for ii in range(5):
                  ...
                  if i0 >= 0 and i0 < num and j0 >= 0 and j0 < num and k0 >= 0 and k0 < num:
                    grid[k0,j0,i0] += d
...
def show_profile(grid_range):
  ...
  show_image_stack_multi_view(grid, vmax=vmax, extent=[min_x,max_x,min_x,max_x],
                            axis_names=["L", "K", "H"])
...
def crystal_parameters():
  ...
  # print ""
  print("SUM(p * dp/dp)")
  MAT = [p.value*db_dp for p, db_dp in zip(parameters.get_params(),
                                          parameters.get_ds_dp())]
  COV = sum(MAT[1:], MAT[0])
  print(2*COV.round(7))
