...
/* using
 * dials::algorithms::profile_model::gaussian_rs::transform::TransformSpec; */
/* using dials::algorithms::profile_model::gaussian_rs::CoordinateSystem; */
/* using
 * dials::algorithms::profile_model::gaussian_rs::transform::TransformReverseNoModel;
 */
/* using
 * dials::algorithms::profile_model::gaussian_rs::transform::TransformReverse;
 */
/* using
 * dials::algorithms::profile_model::gaussian_rs::transform::TransformForward;
 */
...
/*     /1* for (iterator it = lookup_refls_.begin(); it != lookup_refls_.end();
 * ++it) { *1/ */
...
/*     for (iterator it = lookup_count_.begin(); it != lookup_count_.end();
 * ++it) { */
...
/*       Summation<double> summation(r_data.const_ref(), r_bgrd.const_ref(),
 * r_mask.const_ref()); */
...
/*       /1*   ProfileFitting<double> fitting(profile.const_ref(),
 * r_mask2.const_ref(), r_data.const_ref(), r_bgrd.const_ref(), 1e-3, 10); *1/
 */
...
/*   typedef boost::unordered_map< std::size_t, std::vector< std::size_t > >
 * lookup_refls_type; */
...
/*   virtual void operator()(af::Reflection &reflection, bool adjacent=false)
 * const = 0; */
...
/*                           const std::vector<af::Reflection>
 * &adjacent_reflections) const = 0; */
...
/*     bool operator()(int a) const { */
/*       return ((a & mask_code) == mask_code || (a & (Valid | Overlapped)) ==
 * (Valid | Overlapped)); */
/*     } */
/*   }; */
...
/*     int mask_code1; */
/*     int mask_code2; */
/*     check_either_mask_code(int code1, int code2) : mask_code1(code1),
 * mask_code2(code2) {} */
/*     bool operator()(int a) const { */
/*       return ((a & mask_code1) == mask_code1) || ((a & mask_code2) ==
 * mask_code2); */
/*     } */
/*   }; */
...
/*     reflection["flags"] = reflection.get<std::size_t>("flags") &
 * ~af::IntegratedPrf; */
 ...
 * background(sbox.background.accessor()); */
/*     std::copy( */
...
/*       if ((sbox.mask[i] & (Valid | Foreground | Overlapped)) == (Valid |
 * Foreground | Overlapped)) { */
/*         is_overlapping = true; */
/*         break; */
/*       } */
/*     } */
 ...
 */
/*         double phi2 = adjacent_reflections[j].get< vec3<double>
 * >("xyzcal.mm")[2]; */
...
/*       /1* std::cout << "Num reflections " << adjacent_reflections.size() <<
 * std::endl; *1/ */
...
/*       /1*     for (std::size_t j = 0; j < m.accessor()[1]; ++j) { *1/ */
/*       /1*       for (std::size_t i = 0; i < m.accessor()[2]; ++i) { *1/ */
/*       /1*         std::cout << profile(af::tiny<std::size_t,4>(l,k,j,i)) <<
 * ", "; *1/ */
/*       /1*       } *1/ */
/*       /1*       std::cout << std::endl; *1/ */
/*       /1*     } *1/ */
...
/*    * @param index The image index */
/*    *1/ */
/*   void copy(const Image<double> &data, const Image<bool> &mask, std::size_t
 * index) { */
...
/*       adjacent_reflections[i]["shoebox"] = reflection.get< Shoebox<>
 * >("shoebox"); */
...
/*     std::vector< std::vector<af::Reflection> >
 * adjacent_reflections(reflections.size()); */
/*     for (std::size_t i = 0; i < reflections.size(); ++i) { */
...
/*         for (AdjacencyList::edge_iterator it = edges.first; it !=
 * edges.second; ++it) { */
...
BOOST_PYTHON_MODULE(dials_scratch_jmp_fitting_ext) {
  ...
  /* class_<MaskCalculatorIface, boost::noncopyable>("MaskCalculatorIface",
   * no_init) */
  /*   ; */

  /* class_<BackgroundCalculatorIface,
   * boost::noncopyable>("BackgroundCalculatorIface", no_init) */
  /*   ; */

  /* class_<IntensityCalculatorIface,
   * boost::noncopyable>("IntensityCalculatorIface", no_init) */
  /*   ; */

  /* class_<MaskCalculator, bases<MaskCalculatorIface> >("MaskCalculator",
   * no_init) */
  ...
  /* class_<BackgroundCalculator, bases<BackgroundCalculatorIface>
   * >("BackgroundCalculator", no_init) */
  ...
  /* class_<IntensityCalculator, bases<IntensityCalculatorIface>
   * >("IntensityCalculator", no_init) */
  /*   .def(init< */
  /*       const Reference&, */
  /*       const CircleSampler&, */
  /*       const TransformSpec&, */
