...
public:
  ...
  generic_tree_node(const_reference value) : value_(value) {}
  ...
  reference front() { return base_type::front().value_; }

  const_reference front() const { return base_type::front().value_; }

  reference back() { return base_type::back().value_; }

  const_reference back() const { return base_type::back().value_; }
  ...
  void pop_back() { base_type::pop_back(); }

  void pop_front() { base_type::pop_front(); }
  ...
  public:
    ...
    iterator(node_iterator iter) : node_(iter) {}

    bool operator!() { return !node_; }
    ...
    pointer operator->() { return &node_->value_; }
    ...
    reference front() { return node_->front(); }

    const_reference front() const { return node_->front(); }

    reference back() { return node_->back(); }

    const_reference back() const { return node_->back(); }

    iterator push_back(const_reference v) { return node_->push_back(v); }

    iterator push_front(const_reference v) { return node_->push_front(v); }

    void pop_back() { node_->pop_back(); }

    void pop_front() { node_->pop_front(); }

    iterator insert(iterator position, const_reference v) {
      return node_->insert(position, v);
    }

    iterator erase(iterator position) { return node_->erase(position); }
    ...
    node_iterator node() { return node_; }

    const_node_iterator node() const { return node_; }
    ...
    iterator first_child() { return begin(); }

    const_iterator first_child() const { return begin(); }

    iterator last_child() { return --end(); }

    //      const_iterator last_child() const {
    //        return --end();
    //      }

    bool leaf() const { return empty(); }
